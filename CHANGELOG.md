...
- AI flow updates
- AI flow updates
- implement klogd Omnipresence via Gemini KV context cache
- AI flow updates
- AI flow updates
- AI flow updates
- AI flow updates
...
- hot-reload capability signal for Creative Liberation Engine instruction layer
...
- add productionCaseToScribeInput mapper -- unified DIRA->klogd tag contract (#4)
- implement Airbyte ingestion background worker to Chromadb
- add CreatorSidebar and CreatorProductivityDashboard components
- SC-04 kstated v2 boot protocol w/ HANDOFF.md task-aware recall
- klogd v2 memory layer + MCP Autoloader
- 5-helix sprint — MCP-05/06/07 + TOOL-01/02/03 + COMET dispatch
- DIRA auto-resolve engine — DIRA-01, DIRA-02, DIRA-03 [partial]
- SC-05 wire klogd tools into AVERI flows + SC-06 integration tests
- complete klogd v2 + MCP Router epics [SC-01..06, MCP-01..04]
- scaffold MCP Router package + claim tasks [T20260307-772, T20260307-886]
- klogd v2 context-pager + kstated-boot scaffolds, ghost updates, pnpm lockfile sync
- design-agent scanner, god-prompt pipelines, ghost SEO, genkit tsconfig cleanup, linter fixes [multi-helix]
- parallelize Creative Liberation Engine execution
- agent runtime implementations + genkit + css updates
- update flow index exports, cle-core index, EngineStatusGrid component
...
- clean git state — config/env module, kstrigd-bridge, engine grid, agent-extension; exclude workspace-mcp nested repo
...
- add @cle/auth to genkit + god-prompt Dockerfiles to resolve ERR_MODULE_NOT_FOUND at runtime
- fix genkit TSX monorepo resolution and ghost missing xml2js dependency

### 🔩 Other
- rename Creative Liberation Engine and Antigravity to CLE globally
- miscellaneous updates
...
- Redis pub/sub auto-chain — brief.created fires on intake complete, campaign auto-executes [T20260306-011]
- add IDEATE, SHIP, VALIDATE, klogd skill files + update dispatch queue
- AGENTS.md, .agents rules, dispatch board, CONTEXT.md in key packages
...
- miscellaneous updates
- miscellaneous updates
- miscellaneous updates — packages/cle-core/src/index.ts, packages/mcp-fetch-proxy/src/mcp-server.ts, packages/klogd-mcp/src/mcp-server.ts
- miscellaneous updates — packages/cle-core/tsconfig.json, packages/mcp-fetch-proxy/package.json, packages/mcp-router/package.json, packages/mcp-router/src/mcp-server.ts
- miscellaneous updates — packages/cle-core/src/types/home-mesh.ts, packages/sovereign-home-mesh/
- miscellaneous updates — packages/cle-core/src/index.ts
- miscellaneous updates — packages/cle-core/src/types/psi.ts
- clean gitignore + sweep all multi-helix sprint output
- remove T20260306-A3 from active queue
- sync all active windows — multi-instance batch commit
...
- add cle-dispatch MCP server — task routing via stdio #30
...
- add cle-klogd MCP server — remember, recall, context, forget, handoff (Phas
...
- Wave 21: Fix orchestrator AgentRouter test + verify T20260309-216 Wave 18 helices
- miscellaneous updates — packages/cle-core/src/index.ts, packages/mcp-fetch-proxy/src/mcp-server.ts, packages/klogd-mcp/src/mcp-server.ts
- Add packages/klogd-mcp/tsconfig.json
...
- add klogd-daemon upstream :9100 /klogd/ route + /provision pass-through; all 9 GENESIS services now behind gateway
- all-green dashboard — live service health hook, NeuralMonitor real endpoints, CampaignControl live status, dispatch activity feed [T20260306-012]
- Redis pub/sub auto-chain — brief.created fires on intake complete, campaign auto-executes [T20260306-011]
- Universal Agent Dispatch Network - Phase 1+2+3
...
### 🔩 Other
- SHIP apps/console: Redesign Tactical HUD split-pane dashboard with Glass Protocol
- Apply Wave 40 and rebase remote divergence
- anonymize repository identity to collective 'we' instead of personal
- stage all pending changes — console, auth, genkit server, memory pkg, pnpm-lock, velocity telemetry page
- scrub simulation nomenclature for zero day production release
- pnpm lockfile sync
- AGENTS.md updates
- merge: creative-liberation-engine sync — Dashboard + nas-watcher + zero-day deps
