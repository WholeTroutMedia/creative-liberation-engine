...
const OLLAMA_HOST = normalizeHost(envKey('OLLAMA_HOST') ?? '192.168.2.20:11434');
...
function buildProviderChain(taskType: TaskType): Array<{ name: string; fn: CallFn }> {
    ...
    const chains: Record<TaskType, Array<keyof typeof providers>> = {
        coding: ['anthropic', 'geminiPro', 'gemini', 'ollama'],
        research: ['geminiPro', 'anthropic', 'gemini', 'ollama'],
        fast: ['gemini', 'anthropic', 'ollama'],
        legal: ['anthropic', 'geminiPro', 'ollama'],
        general: ['geminiPro', 'anthropic', 'gemini', 'ollama'],
    };
...
export async function executeClaudeTask(task: AgentTask): Promise<AgentResult> {
    ...
    console.log(`[claude-agent] Keys: anthropic=${!!KEYS.anthropic} gemini=${!!KEYS.gemini}`);
