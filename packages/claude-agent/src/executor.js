/**
 * @cle/claude-agent â€” Smart Provider Executor v2
 *
...
const DISPATCH_URL = process.env['DISPATCH_SERVER'] ?? 'http://127.0.0.1:5150';
const REPO_ROOT = process.env['CLE_REPO_ROOT']
    ?? 'D:\\Google Antigravity\\Infusion Engine Brainchild\\creative-liberation-engine';
const DEFAULT_TOOLS = ['Read', 'Edit', 'Bash', 'Glob', 'LS'];
...
const STATS = {};
function trackCall(provider, latency, error) {
    if (!STATS[provider]) {
...
async function callGemini(prompt, system, modelOverride) {
...
async function callPerplexity(prompt, system, modelOverride) {
...
function detectTaskType(task) {
    ...
    if (/\b(fix|debug|implement|refactor|typescript|code|build|deploy|docker)\b/.test(text))
        return 'coding';
    if (/\b(research|search|find|explore|discover|news|trend|current)\b/.test(text))
        return 'research';
    if (/\b(legal|contract|compliance|tos|privacy|gdpr)\b/.test(text))
        return 'legal';
    if (/\b(classify|tag|label|extract|parse|format)\b/.test(text))
        return 'fast';
    return 'general';
}
function buildProviderChain(taskType) {
    const providers = {
...
async function generateWithSmartFallback(prompt, system, taskType) {
    ...
    logStats();
    throw new Error(`[claude-agent] All providers exhausted (${taskType}):\n${errors.map((e, i) => `  ${i + 1}. ${e}`).join('\n')}`);
}
// â”€â”€â”€ Public API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function executeClaudeTask(task) {
...
const PERSONA = {
    ...
    general: 'You are IRIS — the Executor of the AVERI collective within the Creative Liberation Engine v6.',
};
...
function buildAgentPrompt(task) {
    ...
    lines.push('', '## Available Tools', (task.tools ?? DEFAULT_TOOLS).map(t => `- ${t}`).join('\n'));
    return lines.join('\n');
}
async function fireHeartbeat(task) {
    const taskType = detectTaskType(task);
