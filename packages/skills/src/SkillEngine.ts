...
export class SkillCompiler {
  ...
  toYAML(pack: SkillPack): string {
    const lines: string[] = [
      ...
      `triggers:`,
      ...pack.metadata.triggers.map((t: string) => `  - ${t}`),
      `capabilities:`,
      ...pack.metadata.capabilities.map((c: string) => `  - ${c}`),
      `dependencies:`,
      ...pack.metadata.dependencies.map((d: string) => `  - ${d}`),
      `instructions: |`,
      ...pack.instructions.split('\n').map((l: string) => `  ${l}`),
    ];
    if (pack.constraints.length > 0) {
      lines.push(`constraints:`);
      pack.constraints.forEach((c: string) => lines.push(`  - ${c}`));
    }
    return lines.join('\n');
  }
...
export class SkillRegistry {
  ...
  search(query: string): SkillPack[] {
    ...
    for (const versions of this.skills.values()) {
      for (const pack of versions.values()) {
        const match = pack.metadata.tags.some((t: string) => t.includes(q))
          || pack.metadata.capabilities.some((c: string) => c.includes(q))
