/**
 * Wire Ingestion MCP — MCP Server
 *
 * Exposes three tools to the Creative Liberation Engine:
 *   - wire.query  — search by topic, category, source, time
 *   - wire.latest — get newest entries
 *   - wire.stats  — ingestion health
 */
...
const server = new Server(
  { name: 'wire-ingestion-mcp', version: '6.0.0' },
  { capabilities: { tools: {} } },
);
  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          since: { type: 'string', description: 'ISO 8601 datetime — only return entries after this time' },
        },
    ...
    {
      name: 'wire_stats',
      description: 'Get ingestion health stats — how many articles fetched, new entries, errors, last run time.',
...
async function main(): Promise<void> {
  ...
  console.error(`[wire-mcp] Server ready — ${FEED_COUNT} feeds ingesting`);
