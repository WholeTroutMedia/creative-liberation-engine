...
async function ingestFeed(source: FeedSource): Promise<void> {
  try {
    ...
    if (newCount > 0) {
      console.error(`[wire] ${source.name}: +${newCount} new / ${entries.length} fetched`);
    }
  } catch (err) {
    stats.errors++;
...
export async function startIngester(): Promise<void> {
  console.error(`[wire] 🚀 Starting ingestion for ${FEED_COUNT} feeds across 10 categories`);
  ...
  console.error(`[wire] ✅ All ${FEED_COUNT} feeds scheduled`);
}
...
export function stopIngester(): void {
  ...
  console.error('[wire] Ingester stopped.');
}
