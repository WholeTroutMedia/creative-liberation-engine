...
//   4. Compliance scanner reports 0 literals in @cle/ui source
...
describe('Token Round-Trip Integration', () => {
    ...
    describe('3. Theme Engine Integration', () => {
        it('all 4 themes should be importable', async () => {
            ...
            const { themes } = await import('@cle/theme-engine').catch(() => ({ themes: null }));
            if (!themes) {
        ...
        it('each theme should pass validation when built', async () => {
            const { themes, validateTheme } = await import('@cle/theme-engine').catch(() => ({ themes: null, validateTheme: null }));
    ...
    describe('4. UI Package — Zero Literal Values', () => {
        it('@cle/ui source should have no hex color literals in TSX', () => {
            const violations = [];
    ...
    describe('5. VERA-DESIGN Agent', () => {
        it('design-agent scanner module should be importable', async () => {
            const agent = await import('@cle/design-agent').catch(() => null);
            if (!agent) {
