...
export function okResult<T>(value: T): Result<T, any> {
    return { success: true, value };
}
export function errResult<E>(error: E): Result<any, E> {
    return { success: false, error };
}


// Memory Contract Interface aligned with docs/MEMORY_SPINE.md
...
export class EdgeSyncBridge {
     ...
     */
    public async cacheSnapshotLocally(records: MemoryContract[]): Promise<Result<number, string>> {
        try {
     ...
     */
    public async synchronizeWithNAS(): Promise<Result<{ synced: number; errors: string[] }, string>> {
        try {
            if (!fs.existsSync(this.localDbPath)) {
            ...
            const localRecords: MemoryContract[] = JSON.parse(rawLocal);
...
export class SubquadraticContextCompactor {
     ...
     */
    public async compactDialogue(dialogueHistory: Array<{ role: string; content: string }>, maxTurns: number = 6): Promise<Result<MemoryContract, string>> {
        try {
...
${rawText}`;
            ...
            const compactedRecord: MemoryContract = {
...
export class StaticESMCompiler {
     ...
     */
    public async compileKnowledgeLayer(): Promise<Result<{ compiledFiles: string[]; totalRecords: number }, string>> {
        try {
            if (!fs.existsSync(this.sourceDir)) {
                return errResult(`Source documentation directory not found: ${this.sourceDir}`);
            }

            const compiledMap: Record<string, any> = {};
            let recordCount = 0;
