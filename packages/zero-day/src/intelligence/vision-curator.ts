...
export const visionCuratorFlow: any = ai.defineFlow({
  name: 'visionCurator',
  inputSchema: CurationInputSchema,
  outputSchema: CurationOutputSchema,
}, async (input: z.infer<typeof CurationInputSchema>) => {
...
Context: ${JSON.stringify({ dna: input.project_dna, audience: input.target_audience })}`;
  ...
  for (let i = 0; i < input.images.length; i += 5) {
    ...
    const promises = batch.map(async (img: { id: string; url: string; mimeType: string }) => {
      try {
