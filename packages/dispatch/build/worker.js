 ...
 */
import { getTasks, saveTask, getAgent, saveAgent, ensureStore } from './store.js';
...
let tasksFailed = 0;
async function routeAndExecute(task) {
...
async function runCampaignTask(task) {
    try {
        ...
        if (campaignIdMatch) {
            ...
            if (!res.ok)
                throw new Error(`Campaign execute returned ${res.status}`);
...
// GENKIT HTTP HELPER
// ─────────────────────────────────────────────────────────────────────────────
async function callGenkit(endpoint, body) {
    try {
        ...
        if (!res.ok)
            throw new Error(`Genkit ${endpoint} returned ${res.status}`);
        const data = await res.json();
...
async function markTaskDone(task, result) {
    const now = new Date().toISOString();
    ...
    if (result.output)
        task.handoff_note = result.output.slice(0, 500); // cap note length
    await saveTask(task);
