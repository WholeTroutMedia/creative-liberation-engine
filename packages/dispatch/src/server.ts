// Creative Liberation Engine Dispatch Server — Main Entry Point
// Express HTTP + MCP over SSE transport
// Runs on NAS at 127.0.0.1:5160
// Any MCP-compatible agent connects here — no workspace, no config needed
...
const PORT = parseInt(process.env.PORT ?? '5160');
...
// ── Blocker Store (in-memory + broadcast) ─────────────────────────────────────
...
const TOOLS = [
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                note: { type: 'string', description: 'What was done — commit hash, file paths, summary' },
    ...
    {
        name: 'get_status',
        description: 'Get the full dispatch board — all active agents, queued/active tasks, sessions.',
...
// MCP over SSE — one transport per client session
app.get('/sse', async (req, res) => {
    ...
    req.on('close', () => {
        ...
        console.log(`[dispatch] Agent disconnected — session ${sessionId}`);
    });

    console.log(`[dispatch] Agent connected via SSE — session ${sessionId}`);
});
...
// Agent-to-Agent (A2A) Registry in-memory fallback helper
...
// REST API — plain HTTP for non-MCP clients
...
// REST API — Agent-to-Agent Dispatch Stub (Wave 17)
...
// REST API — Peripheral Sovereign Identity (PSI)
...
// REST API — DIRA resolution metrics
...
// ── SSE Event Stream (Real-time console) ───────────────────────────────────────────
...
// GET /api/events — SSE stream for live console updates
// POST /api/events — Broadcast custom event to all connected SSE clients
app.post('/api/events', (req, res) => {
    const { type, source, payload } = req.body;
    broadcastEvent(type || 'event', { source, payload, timestamp: new Date().toISOString() });
...
app.get('/api/events', async (req, res) => {
    ...
    console.log(`[dispatch:sse] Client connected — ${clientId} (${sseClients.size} total)`);
    ...
    req.on('close', () => {
        ...
        console.log(`[dispatch:sse] Client disconnected — ${clientId} (${sseClients.size} remaining)`);
    });
});
...
// Wrap with broadcast — safe even if broadcast throws
...
// POST /api/tasks/:id/resolve — force-complete a task without requiring it to be claimed first.
...
// POST /api/tasks/:id/complete — normal completion of a claimed task.
// Body: { agent_id: string, note?: string, artifacts?: string[] }
...
// ── Capability Hot-Reload Endpoints ───────────────────────────────────────────
// POST /api/capabilities/broadcast — announce a capability change to all SSE clients.
...
app.post('/api/capabilities/broadcast', (req, res) => {
    ...
    console.log(`[dispatch:capability] Broadcast fired — hash=${currentCapabilityVersion.hash} files=[${changed_files.join(', ')}] source=${source}`);
    res.json({ success: true, version: currentCapabilityVersion, sse_clients_notified: sseClients.size });
});

// GET /api/capabilities/version — returns current capability version hash.
...
// ── Smart Task Pickup ──────────────────────────────────────────────────────────
// GET /api/tasks/next — returns the highest-priority unclaimed task that matches
...
// GET /api/blockers — list blockers, filter by status and/or severity
...
// POST /api/blockers — file a new blocker
app.post('/api/blockers', async (req, res) => {
    ...
    console.log(`[dispatch:blockers] OPEN ${severity} ${type} — ${id} by ${filed_by}`);
    res.json({ success: true, blocker });
});

// POST /api/blockers/:id/claim — IDE agent claims a blocker to work on it
...
// POST /api/blockers/:id/resolve — mark a blocker resolved with a note
...
// GET /api/archive — query tasks_archive (done/failed tasks swept from the hot queue)
...
// POST /api/archive/sweep — manually trigger the weekly sweep
...
// GET /api/agents — list all agents with live stale detection
...
// GET /api/agents/:id/tabs — return the tab manifest last uploaded by this agent
...
// DELETE /api/agents/:id — disconnect/remove an agent from the registry
...
// POST /api/agents/heartbeat — fire-and-forget from every IDE window on every response
...
// POST /api/federation/peer — register a new peer IE dispatch
...
// GET /api/federation/peers — list all peers with health summaries
app.get('/api/federation/peers', (_req, res) => {
    const peers = getPeerHealthSummaries();
    res.json({ count: peers.length, peers });
});

// GET /api/federation/peers/full — full peer objects (admin use)
app.get('/api/federation/peers/full', (_req, res) => {
    const peers = getAllPeers();
    res.json({ count: peers.length, peers });
});

// GET /api/mesh-health — live health snapshot of all federated peers
app.get('/api/mesh-health', (_req, res) => {
    res.json(buildMeshHealthSnapshot());
});

// DELETE /api/federation/peer/:peerId — remove a peer
...
async function main() {
    ...
    // Start federation gossip loop — pings peers every 30s and broadcasts mesh_health
    ...
    const httpServer = app.listen(PORT, '0.0.0.0', () => {
        console.log(`\n╔══════════════════════════════════════════════╗`);
        console.log(`║  CLE DISPATCH SERVER — ONLINE         ║`);
        console.log(`║  http://127.0.0.1:${PORT}                ║`);
        console.log(`║  MCP: GET /sse  |  REST: GET /api/status    ║`);
        console.log(`║  Federation: POST /api/federation/peer      ║`);
        console.log(`║  Mesh Health: GET /api/mesh-health  (SSE)   ║`);
        console.log(`╚══════════════════════════════════════════════╝\n`);
    });

    const wss = new WebSocketServer({ server: httpServer, path: '/ws/bridge' });
    const activeBridgeSockets = new Set<import('ws').WebSocket>();

    wss.on('connection', (ws) => {
        console.log(`[dispatch] CLE Bridge connected via WebSocket`);
        activeBridgeSockets.add(ws);
            ...
            try {
                ...
                if (msg.type === 'register') {
                    ...
                    const ackPayload = JSON.stringify({
                        action: 'reply_ui',
                        payload: { 
                            html: `<div style="color: #888;"><strong>[DISPATCH]</strong> Task queued: <code>${taskId}</code><br/>Command: <em>${msg.command}</em></div>` 
                        }
                    });
                    ws.send(ackPayload);
        ...
        ws.on('close', () => {
            activeBridgeSockets.delete(ws);
            console.log(`[dispatch] CLE Bridge disconnected`);
        });
    });

    // -- Bridge UI Renderer --------------------------------------------------
    app.post('/api/bridge/render', (req, res) => {
        const { html, css } = req.body;
        if (!html && !css) {
            res.status(400).json({ error: 'Missing html or css in payload' });
            return;
        }
        ...
        let sentCount = 0;
        for (const client of activeBridgeSockets) {
        ...
        res.json({ success: true, clients_notified: sentCount });
    });

    // -- Bridge Native Chat Reply --------------------------------------------
    app.post('/api/bridge/reply', (req, res) => {
        const { text, html, css } = req.body;
        
        const payload = JSON.stringify({
            action: 'reply_ui',
            payload: { text, html, css }
        });
