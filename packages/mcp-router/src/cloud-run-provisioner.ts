...
export class CloudRunProvisioner {
  ...
  constructor(options?: {
    ...
    this.projectId  = options?.projectId  ?? process.env['GCP_PROJECT_ID']    ?? 'inception-engine-prod';
    this.defaultImage = options?.defaultImage ?? process.env['CLOUD_RUN_BASE_IMAGE'] ?? 'gcr.io/inception-engine-prod/inception-runtime:latest';
    this.dispatchUrl = options?.dispatchUrl  ?? process.env['DISPATCH_SERVER'] ?? 'http://122.0.3.1:5150';
  }
  ...
  async provisionTenant(spec: CloudRunServiceSpec): Promise<CloudRunServiceRecord> {
    ...
    if (this.projectId !== 'inception-engine-local') {
