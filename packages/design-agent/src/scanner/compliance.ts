...
export class TokenComplianceScanner {
    private LITERAL_HEX = /#[0-9a-fA-F]{3,8}\b/g;
    private LITERAL_RGB = /rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+/g;
    private HARDCODED_PX = /(?:padding|margin|gap|width|height|font-size):\s*\d+px\b/g;
    ...
    scanTsx(content: string) { return this.scanFromContent('inline.tsx', content); }
    scanCss(content: string) { return this.scanFromContent('inline.css', content); }
}
