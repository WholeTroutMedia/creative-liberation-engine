...
export interface HarmonyResult {
    ...
    issues: string[];
    details: {
        hueCoherence: number;       // 0-100: how tightly clustered the hues are
        saturationCoherence: number; // 0-100: consistency of saturation
        monotonicLightness: boolean; // whether lightness decreases monotonically
    };
}
...
export interface PaletteReport {
    ...
    contrastMatrix: Array<{ fg: string; bg: string; ratio: number; wcagAA: boolean; wcagAAA: boolean }>;
}
...
export class ColorValidator {
     ...
     */
    static checkContrast(fg: string, bg: string, required = 4.5): ContrastResult {
        try {
     ...
     */
    static checkLightnessMonotonicity(
        scale: string[],
        direction: 'asc' | 'desc' = 'desc',
    ): { passed: boolean; offendingIndices: number[] } {
        ...
        for (let i = 0; i < scale.length; i++) {
            try {
                ...
                if (i > 0) {
                    ...
                    if (violation) offendingIndices.push(i);
                }
                prev = lum;
            } catch {
    ...
    static analyzeFullPalette(colors: string[]): PaletteReport {
        ...
        const contrastMatrix: PaletteReport['contrastMatrix'] = [];
