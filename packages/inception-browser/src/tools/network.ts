    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                savePath: { type: "string", description: "Local directory to save to (default: ~/Downloads/cle-browser/)" },
...
export async function handleNetworkTool(
    ...
    switch (name) {
        ...
        case "browser_download": {
            ...
            const downloadDir = savePath ?? path.join(os.homedir(), "Downloads", "cle-browser");
        ...
        case "browser_har_export": {
            ...
            const harPath = savePath ?? path.join(os.homedir(), ".cle-browser", `har-${Date.now()}.json`);
        ...
        case "browser_set_proxy": {
            ...
            return {
                content: [{
                    type: "text" as const,
                    text: `Note: Proxy must be set at browser launch. Restart cle-browser with CLE_PROXY=${server} env var. Current session unaffected.`,
                }],
            };
        }
