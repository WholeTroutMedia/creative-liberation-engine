...
export const LOCAL_MODELS = {
    ...
    multimodal: resolveModel('local:multimodal'),
} as const;
...
export async function localGenerate(options: {
    ...
    const modelName = LOCAL_MODELS[capability];
    const modelStr = modelName.startsWith('nvidia/') ? modelName : `ollama/${modelName}`;

    const icon = modelName.startsWith('nvidia/') ? '🟢 NVIDIA' : '🦙 Ollama';
    console.log(`[LOCAL] ${icon} | ${modelStr} | ${prompt.slice(0, 60)}...`);
...
export async function checkOllamaHealth(): Promise<{
    ...
    try {
        ...
        const ollamaHost = process.env.OLLAMA_HOST ?? 'http://192.168.2.25:11434';
...
export async function sovereignGenerate(
    ...
    if (modelName.startsWith('nvidia/')) {
        try {
            const { checkNvidiaHealth } = await import('./plugins/nvidia.js');
            const nimHealth = await checkNvidiaHealth();
            isOnline = nimHealth.online;
            healthReason = `NVIDIA NIM online on ${nimHealth.host} | served models: ${nimHealth.models.join(', ')}`;
        } catch (err) {
            isOnline = false;
            healthReason = `NVIDIA NIM error: ${(err as Error).message}`;
        }
    } else {
        const health = await checkOllamaHealth();
        isOnline = health.online;
        healthReason = `Ollama online | GPU: ${health.gpuActive}`;
    }

    if (isOnline) {
        try {
            logSovereigntyDecision('local', capability, healthReason);
    ...
    // 2. Cloud fallback — Local offline or errored
    logSovereigntyDecision('cloud-fallback', capability, `Local inference offline (${healthReason}) — sovereign preference fallback to cloud`);
