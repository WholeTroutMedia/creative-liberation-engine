...
const BoltInputSchema = z.object({
    ...
    repoPath: z.string().default(`d:\\Google Creative Liberation Engine\\Infusion Engine Brainchild\\creative-liberation-engine-v5`),
...
After implementing, return the primary code artifact and list of modified files.${memoryContext}`;
            ...
            const { output } = await ai.generate(await withHumanState(applyOmnipresenceCache({
