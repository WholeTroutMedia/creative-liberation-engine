...
import { getHumanStateContext, serializeHumanStateForPrompt } from '@cle/sensor-mesh';
...
async function getDispatchContext(): Promise<string> {
  try {
    const res = await fetch('http://127.0.0.1:5150/api/status');
...
async function getWtmContext(clientId: string): Promise<string> {
  const profiles: Record<string, string> = {
    ...
    'e-is-for-eat':  'Client: E Is For Eat (Jaymee Sire). Food, travel, lifestyle content. YouTube, Instagram, brand partnerships. Media at W:\\E Is For Eat.',
    ...
    'wtm-internal':  'WholeTroutMedia Studio. Creative production arm serving Barnstorm, E Is For Eat, and Creative Liberation Engine. All client work and media production coordinated here.',
  };
...
const AVERI_SYSTEM_PROMPT = `You are AVERI â€” the creative intelligence of WholeTroutMedia and the Creative Liberation Engine.
...
- WholeTroutMedia Studio (WTM): a creative production house working across photography, video, food content, tech
- E Is For Eat: food and travel brand owned by Jaymee Sire, built on years of culinary content and network television
- Barnstorm Photo + Video: a wedding and lifestyle photography studio â€” Artist's primary creative partner
...
export const averiChatFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const [dispatchCtx, clientCtx, intentResult, humanState] = await Promise.all([
      getDispatchContext(),
      getWtmContext(input.clientId ?? 'wtm-internal'),
      Promise.resolve(classifyResearchIntentFast(input.message)),
      getHumanStateContext(),  // ANTITRUST — never blocks, 2s timeout
    ]);

    // Derive model tier from operator biometric state
    const defaultModel = process.env.GENKIT_DEFAULT_MODEL || 'googleai/gemini-2.5-flash';
    const resolvedModel = (() => {
      if (!process.env.BIOMETRIC_ROUTING_ENABLED) return defaultModel;
      if (humanState.routingHint === 'deep') return 'googleai/gemini-2.5-pro';
      if (humanState.routingHint === 'lightweight') return 'googleai/gemini-2.5-flash';
      return defaultModel;
    })();
    if (resolvedModel !== defaultModel) {
      console.log(`[AVERI] 🫀 Biometric routing → model=${resolvedModel} (hint=${humanState.routingHint} | mood=${humanState.mood})`);
    }
    ...
    if (intentResult?.needsResearch) {
      try {
        const research = await webResearchFlow({
          query: intentResult.searchQuery || input.message,
          depth: 'deep',
        });
...
IMPORTANT: Use the research above to answer accurately. Synthesize it in AVERI's voice. Always acknowledge information is from live web sources.`;
          ...
          console.log(`[AVERI] ✓ Research injected | model:${research.model} | citations:${citations.length}`);
        } else {
          console.warn(`[AVERI] Web research failed, proceeding LLM-only: ${research.errorMessage}`);
        }
      } catch (err) {
    ...
    const biometricFragment = serializeHumanStateForPrompt(humanState);
