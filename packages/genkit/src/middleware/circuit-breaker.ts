...
export const FALLBACK_CHAINS = {
    // Maximum reasoning capabilities. Start with Pro, fallback to DeepSeek, then Claude Sonnet
    tier3Max: {
        primaryModel: process.env.GENKIT_PRO_MODEL ?? 'googleai/gemini-2.5-pro',
        fallbackModels: ['deepseek/deepseek-reasoner', 'anthropic/claude-3-7-sonnet'],
        verbose: true
    },
    // Fast cloud tasks. Start with Flash, fallback to Haiku
    tier2Fast: {
        primaryModel: process.env.GENKIT_DEFAULT_MODEL ?? 'googleai/gemini-2.5-flash',
        fallbackModels: ['anthropic/claude-3-5-haiku-latest'],
        verbose: true
    },
    // Local processing. If Gemma 4 27B fails, cascade to the coder model
    tier1Local: {
