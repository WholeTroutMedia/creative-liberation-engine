...
export type LocalTier = 'local:fast' | 'local:code' | 'local:large' | 'local:embed' | 'local:vision' | 'local:multimodal' | 'local:reasoning' | 'local:reasoning:fast';
...
export const MODEL_REGISTRY: Record<ModelTier, string> = {
    ...
    'local:multimodal':     env('MODEL_LOCAL_MULTIMODAL',     'nvidia/nemotron-3-ultra'), // NVIDIA Local NIM (fallback: nemotron3)
    'local:reasoning':      env('MODEL_LOCAL_REASONING',      'deepseek-r1:32b'),         // DeepSeek R1 32B
    'local:reasoning:fast': env('MODEL_LOCAL_REASONING_FAST', 'deepseek-r1:8b'),          // DeepSeek R1 8B
