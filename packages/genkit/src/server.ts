...
import { tripleseatIngestFlow, prismIngestFlow } from './flows/event-ingestion.js';
...
app.get('/dira/metrics', async (req, res) => {
    ...
    try {
        ...
        const docs = await docsRes.json() as { metadatas?: any[] };
...
// ---------------------------------------------------------------------------
// POST /sentinel/ideate -- compatibility alias for Flipboard Sentinel
// ---------------------------------------------------------------------------
app.post('/sentinel/ideate', async (req, res) => {
    try {
        const topic = req.body?.topic ?? req.body?.prompt ?? req.body?.title;
        if (!topic) {
            return res.status(400).json({ error: '"topic", "prompt", or "title" is required' });
        }
        const { context, depth = 'deep', sessionId } = req.body;
        const keeperResult = await KEEPERFlow({
            task: 'search',
            query: topic,
            tags: ['ideate', 'averi', 'sentinel'],
            sessionId,
        });
        const athenaResult = await ATHENAFlow({
            mode: 'strategy',
        ...
        console.error('[SENTINEL:IDEATE] Error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// ---------------------------------------------------------------------------
// POST /tripleseat/ingest -- Ingest and process Tripleseat BEO webhooks
// ---------------------------------------------------------------------------
app.post('/tripleseat/ingest', async (req, res) => {
    try {
        const payload = req.body;
        const result = await tripleseatIngestFlow({ payload });
        res.json(result);
    } catch (error: any) {
        console.error('[TRIPLESEAT:INGEST] Error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// ---------------------------------------------------------------------------
// POST /prism/ingest -- Ingest, process, and audit Prism.fm show webhooks
// ---------------------------------------------------------------------------
app.post('/prism/ingest', async (req, res) => {
    try {
        const payload = req.body;
        const result = await prismIngestFlow({ payload });
        res.json(result);
    } catch (error: any) {
        console.error('[PRISM:INGEST] Error:', error.message);
        res.status(500).json({ error: error.message });
    }
});
