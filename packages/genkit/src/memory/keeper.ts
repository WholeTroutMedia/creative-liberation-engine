...
/**
 * Parse HANDOFF.md at repo root (see `resolveHandoffMdPath`).
...
function parseHandoffMd(): { taskHint: string; workstream: string } | null {
    const handoffPath = resolveHandoffMdPath();
    try {
...
export async function keeperBoot(ctx: KeeperBootContext = {}): Promise<KeeperBootResult> {
    ...
    if (!shouldInvokeKeeperRecall({ taskHint, hadHandoffFile })) {
        return {
            memoryBrief: '',
            alerts: [],
            recalledCount: 0,
    ...
    try {
        ...
        recalledItems = (recallResult.results ?? []).map(r => ({
            content: r.content || '',
