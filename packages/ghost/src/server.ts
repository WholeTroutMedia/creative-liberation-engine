...
import { telnyxSmsWebhook } from './sms-webhook.js';

const app: Application = express();

// Raw body required for Telnyx Ed25519 verification (must match signed bytes exactly).
app.use(
    express.json({
        limit: '10mb',
...
// Telnyx calls this for every inbound SMS to +12198001070.
// Configure in Telnyx portal → Messaging → Messaging Profiles → Webhook URL.
app.post('/webhooks/telnyx/sms', telnyxSmsWebhook);
...
╠══════════════ TELNYX WEBHOOK ════════════════════════════╣
║   POST /webhooks/telnyx/sms     — Inbound SMS handler    ║
╠═══════════════ SMG CRAWL ════════════════════════════════╣
