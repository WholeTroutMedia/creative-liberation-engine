...
async function sendEmail(alert: RelayAlert, to: string): Promise<{ ok: boolean; id?: string; error?: string }> {
    const agentMailKey = getEnv('AGENT_MAIL_API_KEY');
    const agentMailUrl = getEnv('AGENT_MAIL_URL') || 'https://agent-mail.wholetrou.workers.dev/api/send';
  ...
  <div style="max-width:600px;margin:0 auto">
    <div style="background:#1a1a25;border-left:4px solid ${alert.severity === 'critical' ? '#ef4444' : alert.severity === 'high' ? '#f97316' : '#b87333'};padding:24px;border-radius:4px">
      <h2 style="margin:0 0 8px;color:${alert.severity === 'critical' ? '#ef4444' : alert.severity === 'high' ? '#f97316' : '#b87333'}">${SEVERITY_EMOJI[alert.severity]} ${alert.title}</h2>
    ...
    // Priority 1: Cloudflare Worker API
    if (agentMailKey) {
        try {
            ...
            console.error(`[RELAY] Cloudflare send failed:`, e.message);
        }
    }

    // Fallback: Gmail OAuth2 SMTP
    const clientId = getEnv('GMAIL_CLIENT_ID');
    const refresh = getEnv('GMAIL_REFRESH_TOKEN');
    const adminEmail = getEnv('ADMIN_EMAIL') ?? 'inquiries@creativeliberationengine.org';
...
async function sendSMS(alert: RelayAlert, to: string): Promise<{ ok: boolean; sid?: string; error?: string }> {
    ...
    const body = `${SEVERITY_EMOJI[alert.severity]} [CLE/${alert.severity.toUpperCase()}] ${alert.title}\n${alert.body.slice(0, 140)}`;
