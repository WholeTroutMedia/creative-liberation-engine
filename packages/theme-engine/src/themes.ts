...
export interface ThemeValidationResult {
    passed: boolean;
    failures: {
        contrast?: Array<{ pair: string; actual: number; required: number }>;
        hierarchy?: string[];
        spacing?: string[];
    };
    warnings: string[];
...
export function validateTheme(
    ...
    resolvedFontSizes?: Record<string, string>,
): ThemeValidationResult {
    const failures: ThemeValidationResult['failures'] = {};
    ...
    if (resolvedColors && theme.overrides.color) {
        ...
        const criticalPairs: Array<{ pair: string; fg: string; bg: string; uiOnly: boolean }> = [
        ...
        const contrastFailures: NonNullable<ThemeValidationResult['failures']['contrast']> = [];
    ...
    for (const hKey of HEADING_KEYS_ORDERED) {
        ...
        const candidates = [
            ...
            fontSource[hKey],
            (theme.overrides as Record<string, unknown>)[`font.${hKey}.size`] as string | undefined,
        ...
        for (let i = 1; i < headingSizes.length; i++) {
            ...
            if (curr.px >= prev.px) {
                hierarchyViolations.push(
                    `${curr.key} (${curr.px}px) must be smaller than ${prev.key} (${prev.px}px)`,
                );
            } else {
                const ratio = prev.px / curr.px;
                if (ratio < MIN_HEADING_RATIO) {
                    warnings.push(
                        `${prev.key}→${curr.key} ratio ${ratio.toFixed(3)} < ${MIN_HEADING_RATIO} — heading scale is too flat`,
                    );
                }
            }
        }
        if (hierarchyViolations.length > 0) failures.hierarchy = hierarchyViolations;
    ...
    if (theme.overrides.spacing) {
        ...
        if (spacingViolations.length > 0) failures.spacing = spacingViolations;
    }
...
export const BUILT_IN_THEMES: Record<ThemeId, Theme> = {
 ...
 */
export function generateThemeCSS(theme: Theme, selector?: string): string {
