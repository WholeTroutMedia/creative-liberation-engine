// packages/cle-browser/src/tools/forms.ts
// Form interaction tools — fill, submit, validate, select, file upload
...
export const formTools: Tool[] = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                clearFirst: { type: 'boolean', description: 'Clear existing value before filling (default: true)' },
            },
    ...
    switch (name) {
        ...
        case 'browser_form_get_values': {
            ...
            const values = await page.evaluate((sel: string) => {
                ...
                return Array.from(data as unknown as Iterable<[string, FormDataEntryValue]>).map((entry) => ({ name: entry[0], value: String(entry[1]) }));
            }, formSelector);
