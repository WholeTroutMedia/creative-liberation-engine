/**
 * CLE Browser MCP Server — v2.0 Full Tool Suite
 *
 * 13 tool modules — 60+ tools across:
 *   navigation, interaction, extraction, forms, tabs,
 *   network, evaluate, files, governance, intelligence, sessions, cdp, terminal
 *
...
import { terminalTools, handleTerminalTool } from "./tools/terminal.js";
...
const terminalSet = new Set(terminalTools.map(t => t.name));

// ─── MCP Server Factory ────────────────────────────────────────────────────────
export function createCLEBrowserServer() {
    const server = new Server(
        { name: "cle-browser", version: "2.0.0" },
        { capabilities: { tools: {} } }
    );
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (terminalSet.has(name)) return handleTerminalTool(request);
...
async function main() {
    ...
    const { server, toolCount } = createCLEBrowserServer();
    await server.connect(transport);
    // Register browser mesh with dispatch server (fire-and-forget)
    startDispatchHeartbeat();
    console.error(`🌐 CLE Browser MCP v2.0 — ${toolCount} tools live`);
    ...
    console.error(`   Governance: ${governanceTools.length}  Intelligence: ${intelligenceTools.length}  Terminal: ${terminalTools.length}`);
}
