...
const RECORDING_DIR = path.join(os.homedir(), ".cle-browser", "recordings");
...
export class SessionRecorder {
    ...
    private _navigationHandler?: (url: URL) => void;
    ...
    async start(page: Page): Promise<void> {
        ...
        this._navigationHandler = (_url: URL) => {
            this.record(page, "navigate", { url: _url.toString() });
        };
    ...
    static async list(): Promise<Array<{ name: string; recordedAt: string; actionCount: number }>> {
        try {
            ...
            const results: Array<{ name: string; recordedAt: string; actionCount: number }> = [];
