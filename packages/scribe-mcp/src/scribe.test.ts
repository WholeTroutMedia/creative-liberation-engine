...
describe('scribe.filesystem sandboxing', () => {
  const TEST_SANDBOX = path.resolve('.test-sandbox');
  ...
  function validatePathForTest(targetPath: string): string {
    const resolved = path.resolve(TEST_SANDBOX, targetPath);
    if (!resolved.startsWith(TEST_SANDBOX)) {
      throw new Error(`Access Denied: Path traversal detected. Target path '${targetPath}' resolved outside sandbox root '${TEST_SANDBOX}'`);
    }
    return resolved;
  }

  it('allows access to files inside the sandbox root', () => {
    const target = validatePathForTest('subfolder/file.txt');
    expect(target.startsWith(TEST_SANDBOX)).toBe(true);
  });

  it('denies access to paths resolving outside the sandbox root via parent traversal', () => {
    expect(() => validatePathForTest('../secret.txt')).toThrow('Access Denied');
    expect(() => validatePathForTest('../../etc/passwd')).toThrow('Access Denied');
  });
});
