/**
 * generate-token.js — One-time Gmail refresh token generator
...
if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error('❌ GMAIL_CLIENT_ID or GMAIL_CLIENT_SECRET not found in .env');
  process.exit(1);
}
...
const server = http.createServer(async (req, res) => {
  ...
  console.log('\n[token-gen] ✅ Auth code received — exchanging for refresh token...');
  ...
  try {
    ...
    if (!tokenData.refresh_token) {
      console.error('[token-gen] ❌ No refresh_token in response:', JSON.stringify(tokenData, null, 2));
    ...
    console.log(`[token-gen] ✅ Refresh token obtained!`);
    ...
    console.log('[token-gen] Email delivery is now fully configured for tonight 🎬');
    ...
    server.close(() => {
      console.log('[token-gen] Server closed. You are ready to go! 🚀');
      process.exit(0);
    });
...
server.listen(8080, () => {
  console.log('[token-gen] Callback server running on http://localhost:8080');
