...
export class GoogleSparkEngine {
     ...
     */
    async executeMcp(toolName, args) {
        if (!this.mcpClient) {
     ...
     */
    async logTelemetryToSheets(payload) {
     ...
     */
    async compileProjectDoc(docTitle, markdownContent) {
    ...
    async syncTasksToGoogleTasks() {
        ...
        try {
            ...
            const gTaskTitles = new Set(gTasks.map((t) => t.title));
            for (const t of tasks.filter((t) => t.status === 'queued')) {
    ...
    async queryModel(prompt) {
        ...
        try {
            ...
            const response = await fetch(`${ollamaUrl}/api/chat`, {
                ...
                body: JSON.stringify({
                    model: 'deepseek-r1:8b', // Using standard fast local reasoning
            ...
            console.warn('[Spark Engine] Local inference engine offline. Using fast heuristic fallback.');
        }
     ...
     */
    async applyEmailLabel(messageId, labelName) {
        try {
     ...
     */
    async dispatchToCLEQueue(payload) {
        const dispatchUrl = 'http://127.0.0.1:5160/api/tasks';
        try {
            const response = await fetch(dispatchUrl, {
                method: 'POST',
...
//# sourceMappingURL=google-spark-engine.js.map