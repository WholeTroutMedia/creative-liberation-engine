...
async function migrateUserDirectoryTree(): Promise<void> {
    const USER_SCHEMA = `
        ...
        CREATE TABLE IF NOT EXISTS cle_user_integrations (
            id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            uid             TEXT NOT NULL REFERENCES cle_users(uid) ON DELETE CASCADE,
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_user_integrations_uid_platform ON cle_user_integrations(uid, platform);
    `;
...
async function computeSemanticError(expected: string, actual: string): Promise<number> {
    try {
        const url = process.env.OLLAMA_URL ?? 'http://192.168.2.20:11434';
...
// ─── User Integrations Operations (Encrypted) ─────────────────────────────────
...
export async function getUserIntegration(uid: string, platform: string): Promise<UserIntegration | undefined> {
    const { rows } = await db().query(
        `SELECT id, uid, platform, account_id, credentials, metadata, is_active, created_at, updated_at 
         FROM cle_user_integrations 
    ...
    const r = rows[0];
    let decryptedCreds: any = {};
    try {
        decryptedCreds = JSON.parse(decrypt(r.credentials));
    } catch (err) {
        console.error(`[store] Failed to decrypt credentials for user ${uid} on platform ${platform}:`, err);
    }
...
export async function setUserIntegration(
    uid: string,
    platform: string,
    accountId: string | null,
    credentials: any,
    ...
    await db().query(
        `INSERT INTO cle_user_integrations (uid, platform, account_id, credentials, metadata, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6)
        ...
        [uid, platform, accountId, encryptedCreds, JSON.stringify(metadata), now]
    );
}

export async function deleteUserIntegration(uid: string, platform: string): Promise<boolean> {
    const { rowCount } = await db().query(
        `DELETE FROM cle_user_integrations WHERE uid = $1 AND platform = $2`,
        [uid, platform]
    );
    return (rowCount || 0) > 0;
}
