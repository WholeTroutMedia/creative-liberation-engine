...
describe('VectorStore', () => {
  ...
  describe('search', () => {
    ...
    it('should apply recency bias to penalize old entries', () => {
      const oldDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(); // 30 days old
      const recentDate = new Date().toISOString();
      
      // Both have identical embeddings 
      store.store(makeEntry({ timestamp: oldDate, embedding: [1, 0, 0, 0, 0] }));
      store.store(makeEntry({ timestamp: recentDate, embedding: [1, 0, 0, 0, 0] }));
      
      // With bias, the recent one should win easily
      const results = store.search([1, 0, 0, 0, 0], { recencyBias: 0.1 });
      expect(results.length).toBe(2);
      expect(results[0].entry.timestamp).toBe(recentDate);
      expect(results[1].entry.timestamp).toBe(oldDate);
      
      // The score of the old date should be significantly lower
      expect(results[0].score).toBeGreaterThan(results[1].score);
    });
  });
