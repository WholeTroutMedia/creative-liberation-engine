...
const mockChromaClient = {
  ...
  listCollections: vi.fn().mockResolvedValue([{ name: 'cle-memory' }, { name: 'session-archive' }]),
...
class ChromaBackend {
  ...
  constructor(collectionName = 'cle-memory') {
...
describe('ChromaDB Memory Backend', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    backend = new ChromaBackend('cle-memory');
  ...
  describe('Collection management', () => {
    ...
    it('lists all collections from ChromaDB', async () => {
      ...
      expect(collections).toContain('cle-memory');
