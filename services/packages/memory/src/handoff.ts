...
export type HandoffPhase = 'IDEATION' | 'DESIGN' | 'PLAN' | 'SHIP' | 'VALIDATION';
...
import { agenticSocialGraph } from './agentic-graph.js';

// ─── JSON block extraction from HANDOFF.md ───────────────────────────────────

const JSON_BLOCK_RE = /```json\s*([\s\S]*?)\s*```/;

function extractJson(markdown: string): HandoffState | null {
    const match = JSON_BLOCK_RE.exec(markdown);
    if (!match) return null;
    try {
        return JSON.parse(match[1]) as HandoffState;
    } catch {
        return null;
    }
}
...
export class HandoffService {
     ...
     */
    async readHandoff(): Promise<HandoffState | null> {
        try {
     ...
     */
    async writeHandoff(
        state: Omit<HandoffState, 'timestamp'> & { timestamp?: string }
    ): Promise<void> {
        ...
        // Append to log and update the Agentic Social Graph
        await this.logTransition(stamped);
        await agenticSocialGraph.recordInteraction(stamped.from, `PHASE-${stamped.phase}`, 1);
    }

    /**
     * Get full history of HANDOFF.md transitions.
     */
    async getHandoffHistory(): Promise<HandoffState[]> {
        try {
     ...
     */
    async checkAutoResume(): Promise<HandoffState | null> {
        const state = await this.readHandoff();
        if (!state) return null;

        // Auto-resume conditions:
        // - Phase IDEATION from PERPLEXITY → Creative Liberation Engine should DESIGN
        // - Phase SHIP from CLAUDE-CODE → Creative Liberation Engine should VALIDATION
        const shouldResume =
            (state.phase === 'IDEATION' && state.from === 'PERPLEXITY') ||
