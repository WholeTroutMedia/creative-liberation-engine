...
// ── Schemas ──────────────────────────────────────────────
export const MemoryType = z.enum(['episodic', 'semantic', 'procedural', 'working', 'proxy_pointer']);
...
export class VectorStore {
  ...
  search(queryEmbedding: number[], opts?: { topK?: number; type?: MemoryType; agentId?: string; minImportance?: number; recencyBias?: number }): { entry: MemoryEntry; score: number }[] {
    const topK = opts?.topK ?? 10;
    const now = Date.now();
    const bias = opts?.recencyBias ?? 0.0;
    ...
    for (const entry of this.entries.values()) {
      ...
      if (bias > 0) {
        ...
        // Exponential decay based on age and user-provided bias
        const timeWeight = Math.exp(-bias * ageDays);
