...
// ─────────────────────────────────────────────────────────────────────────────
// MEMORY ROOT — points to cle-memory repo on disk
// ─────────────────────────────────────────────────────────────────────────────

const MEMORY_ROOT = process.env.MEMORY_DIR
    || (process.platform === 'win32' 
        ? `d:\\Google Antigravity\\Infusion Engine Brainchild\\creative-liberation-engine-v5\\ecosystem\\cle-memory`
        : `/tmp/memory`);
...
export type MemoryLayerType = 'PROCEDURAL' | 'SEMANTIC' | 'EPISODIC' | 'SYSTEM';

export const SURFACE_TO_LAYER: Record<string, MemoryLayerType> = {
    'operational.sessions': 'PROCEDURAL',
    'operational.agents': 'PROCEDURAL',
    'operational.projects': 'PROCEDURAL',
    'cognitive.decisions': 'SEMANTIC',
    'cognitive.patterns': 'SEMANTIC',
    'cognitive.learnings': 'SEMANTIC',
    'contextual.users': 'EPISODIC',
    'contextual.events': 'EPISODIC',
    'sync_logs': 'SYSTEM',
};
...
// ─────────────────────────────────────────────────────────────────────────────
// GIT SYNC QUEUE — batched commits to cle-memory repo
// ─────────────────────────────────────────────────────────────────────────────
...
export class MemoryBus {
    ...
    async commit(write: MemoryWrite): Promise<MemoryEntry> {
        ...
        const entry: MemoryEntry & { metadata?: any } = {
            id: `mem_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
