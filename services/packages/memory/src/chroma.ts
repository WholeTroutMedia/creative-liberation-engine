...
export class ChromaMemoryClient {
    ...
    constructor() {
        // Use 'path' for chromadb v1.x compatibility; newer SDKs prefer top-level URL
        // @ts-expect-error — chromadb typings vary by version; runtime arg is accepted
        this.client = new ChromaClient(CHROMA_URL);
        this.embedFn = new OllamaEmbeddingFunction();
    }

    private async getCollection(agentName: string, tier: string = 'mid-term'): Promise<Collection> {
