...
export function hashFNV32(input: string): number {
    let hash = 2166136261;
...
export function regexTest(pattern: string, input: string, flags = 'g'): RegexTestResult {
    try {
        ...
        while ((m = re.exec(input)) !== null) {
            matches.push(m[0]);
            if (m.groups) groups.push(m.groups as Record<string, string>);
...
// ─────────────────────────────────────────────────────────────────────────────
// JWT DECODE (no verification — for inspection only)
// ─────────────────────────────────────────────────────────────────────────────
...
export function jwtDecode(token: string): JWTDecodeResult | null {
    try {
        const parts = token.split('.');
        if (parts.length !== 3) return null;
        const decode = (s: string) => JSON.parse(decodeBase64Url(s)) as Record<string, unknown>;
