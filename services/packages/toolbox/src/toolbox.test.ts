...
describe('hashFNV32', () => {
  ...
  it('is deterministic', () => {
    expect(hashFNV32('cle')).toBe(hashFNV32('cle'));
  });
...
describe('urlParse', () => {
  it('parses a full URL into components', () => {
    const result = urlParse('https://cle.io/api/v1?key=abc&env=prod');
    expect(result.valid).toBe(true);
    expect(result.protocol).toBe('https:');
    expect(result.host).toBe('cle.io');
...
describe('urlSlugify', () => {
  it('converts a title to a slug (returns string)', () => {
    ...
    expect(slug).toBe('hello-world-this-is-cle-engine');
  });
...
describe('ALL_TOOLBOX_TOOLS registry', () => {
  ...
  it('hashFnvTool fn returns hash and hex', () => {
    ...
    const result = (hashTool!.fn as (i: { input: string }) => { hash: number; hex: string })({ input: 'cle' });
