 ...
 * @cle/genmedia fallback chain can silently route to local GPU
...
function fluxImageWorkflow(prompt: string, width: number, height: number) {
    return {
        ...
        '7': { class_type: 'SaveImage', inputs: { images: ['6', 0], filename_prefix: `cle_${Date.now()}` } },
    };
}
...
function sdxlTurboWorkflow(prompt: string, width: number, height: number) {
    return {
        ...
        '6': { class_type: 'VAEDecode', inputs: { samples: ['5', 0], vae: ['1', 2] } },
        '7': { class_type: 'SaveImage', inputs: { images: ['6', 0], filename_prefix: `cle_draft_${Date.now()}` } },
    };
}
