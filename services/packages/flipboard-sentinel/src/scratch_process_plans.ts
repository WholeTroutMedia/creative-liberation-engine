...
async function callAveriPlan(topic: string, context: string, sessionId: string) {
    const url = `${CONFIG.genkitBaseUrl}/averi/plan`;
    console.log(`[SCRATCH-PLAN] Calling /averi/plan for topic: ${topic.slice(0, 50)}...`);
...
async function registerTaskOnDispatch(manifest: JobManifest) {
    const url = 'http://127.0.0.1:5160/api/tasks'; // Live dispatch server URL
...
async function main() {
    ...
    for (const manifest of manifests) {
        ...
        try {
            ...
            const planResult = await callAveriPlan(
