 ...
 */
function categoryOverlap(catsA, catsB) {
 ...
 */
export function findRelatedJobs(manifest) {
    const existingManifests = loadAllManifests().filter(m => m.jobId !== manifest.jobId);
    if (existingManifests.length === 0)
        return [];
    const newKeywords = extractKeywords(`${manifest.sourceArticle.title} ${manifest.athenaOutput?.directive || ''} ${manifest.categories.join(' ')}`);
    const results = [];
    for (const existing of existingManifests) {
        if (existing.status === 'ARCHIVED')
            continue;
        if (!existing.sourceArticle?.title)
            continue; // Skip malformed manifests
        const existingKeywords = extractKeywords(`${existing.sourceArticle.title} ${existing.athenaOutput?.directive || ''} ${(existing.categories || []).join(' ')}`);
 ...
 */
export function applyCrossReferences(manifest) {
