...
export async function sendIdeationEmail(manifest: JobManifest, crossRefs: CrossRefResult[] = []): Promise<void> {
    ...
    const statusEmoji = manifest.cleRelevance >= 70 ? '🔥' : manifest.cleRelevance >= 40 ? '⚡️' : '📡';
...
export function buildIdeationEmailHtml(manifest: JobManifest, crossRefs: CrossRefResult[]): string {
    const { jobId, sourceArticle, categories, cleRelevance, athenaOutput } = manifest;
    const athena = athenaOutput!;

    const isHigh = cleRelevance >= 70;
    const isMed = cleRelevance >= 40;
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width: 100%; background: #ffffff;">
    ...
    <tr><td style="padding: 0;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
            <td style="background: ${accent}; padding: 6px 24px; font-size: 10px; font-weight: 700; color: #fff; text-transform: uppercase; letter-spacing: 1.5px;">${relLabel} Â· ${cleRelevance}% Â· ${catStr}</td>
        </tr></table>
    </td></tr>
