 ...
 */
export function writeObsidianNote(manifest: JobManifest, crossRefs: CrossRefResult[] = []): void {
 ...
 */
function buildNoteContent(manifest: JobManifest, crossRefs: CrossRefResult[]): string {
    const { jobId, slug, status, sourceArticle, categories, cleRelevance, athenaOutput } = manifest;
    ...
    lines.push(`cle_relevance: ${cleRelevance}`);
    ...
    if (manifest.ideatedAt) lines.push(`ideated_at: "${manifest.ideatedAt}"`);
    ...
    // ── Status Badge ──
    const statusEmoji: Record<string, string> = { PENDING: '⏳', IDEATED: '💡', PLANNED: '📋', SHIPPED: '🚀', VALIDATED: '✅', ARCHIVED: '🗄️', DISCARDED: '🗑️' };
    lines.push(`> **Status:** ${statusEmoji[status] || '❓'} ${status} | **Relevance:** ${cleRelevance}/100`);
    lines.push('');
    ...
    if (!athena) {
        ...
        // ── VERA Validation ──
        if ((manifest as any).veraOutput) {
            const vera = (manifest as any).veraOutput;
            lines.push('## ⚖️ VERA Validation Check');
            lines.push('');
            lines.push(`> **Verdict:** ${vera.verdict}`);
            lines.push(`> **Confidence:** ${vera.confidence}`);
            if (vera.contradictions && vera.contradictions.length > 0) {
                lines.push(`> **Contradictions Flagged:**`);
                for (const c of vera.contradictions) {
                    lines.push(`> - ${c}`);
                }
            }
            lines.push('');
 ...
 */
export function writeSentinelDashboard(): void {
    const dir = CONFIG.obsidianSentinelDir;
    fs.mkdirSync(dir, { recursive: true });
    ...
    lines.push('');
    lines.push('```dataview');
    lines.push('TABLE status, cle_relevance as "Relevance", source_author as "Author", created_at as "Created"');
    lines.push('FROM "Sentinel"');
    lines.push('WHERE status != "ARCHIVED"');
    lines.push('SORT cle_relevance DESC');
    lines.push('```');
    lines.push('');
 ...
 */
export function readOperatorComments(manifest: JobManifest): string[] {
    ...
    if (commentSection.length < 2) return [];
