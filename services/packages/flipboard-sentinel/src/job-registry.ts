...
export type JobStatus = 'PENDING' | 'PENDING_RETRY' | 'IDEATED' | 'PLANNED' | 'SHIPPED' | 'VALIDATED' | 'ARCHIVED' | 'DISCARDED';

export type SourceType = 'article' | 'twitter' | 'product' | 'unknown';
...
export interface AthenaIdeation {
    directive: string;
    rationale: string;
    options: Array<{
        title: string;
...
export interface JobManifest {
    ...
    categories: string[];
    cleRelevance: number;
    athenaOutput: AthenaIdeation | null;
    relatedJobs: string[];
    obsidianPath: string;
    comments: string[];
    createdAt: string;
 ...
 */
function normalizeManifest(raw: any): JobManifest | null {
    ...
    // Already V1 format
    if (raw.sourceArticle && raw.jobId) return raw as JobManifest;
    ...
    if (raw.version === 2 || (raw.source && raw.id)) {
        ...
        const manifest: JobManifest = {
            ...
            cleRelevance: cls.cleRelevance ?? 0,
            athenaOutput: ath ? {
                directive: ath.directive || '',
                rationale: ath.rationale || '',
                options: (ath.options || []).filter((o: any) => o && typeof o === 'object'),
    ...
    if (raw.sourceArticle?.title && raw.jobId?.startsWith('IE-IDX-')) return raw as JobManifest;
...
// â”€â”€â”€ Counter Management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Seen Articles (Deduplication) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 */
export function filterNewArticles<T extends { guid: string }>(articles: T[]): T[] {
 ...
 */
export function loadAllManifests(): JobManifest[] {
    if (!fs.existsSync(CONFIG.queueDir)) return [];
 ...
 */
export function updateManifest(manifest: JobManifest): void {
 ...
 */
export function sweepStaleIdeations(): number {
