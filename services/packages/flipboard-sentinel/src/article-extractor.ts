 ...
 * via the headless Chromium instance on the NAS (127.0.0.1:9222).
 * Cookie database at /app/cortex-browser-data persists across restarts.
 * 
 * The agent:
...
async function connectCortex() {
    ...
    try {
        ...
        if (!hasAuth) {
            ...
            const runtimePath = process.env.NAS_RUNTIME_PATH || '/app/creative-liberation-engine/runtime';
...
async function extractTwitterThread(url: string): Promise<TwitterThreadData | null> {
    ...
    try {
        ...
        if (!threadData.isAuthenticated) {
            console.log(`[SENTINEL] ⚠️ CORTEX is NOT authenticated to X.com — content may be truncated. Log in via http://127.0.0.1:3100`);
        } else {
...
async function extractTwitterContent(tweetUrl: string, rssDescription: string): Promise<string> {
    ...
    if (!threadData.isAuthenticated) {
        console.log(`[SENTINEL] 🔑 ACTION NEEDED: Log into X.com via CORTEX at http://127.0.0.1:3100 for full thread access`);
    }
...
async function expandTcoLink(tcoUrl: string): Promise<string | null> {
    try {
        ...
        const response = await fetch(tcoUrl, {
            ...
            redirect: 'follow',
            headers: {
                'User-Agent': 'Mozilla/5.0 (compatible; CLEEngine-Sentinel/1.0)',
            },
        });
...
async function searchDuckDuckGo(query: string): Promise<SearchResult[]> {
    ...
    try {
        ...
        const response = await fetch(searchUrl, {
            signal: controller.signal,
            headers: {
                'User-Agent': 'Mozilla/5.0 (compatible; CLEEngine-Sentinel/1.0; +https://cle.engine)',
                'Accept': 'text/html',
...
export function classifyArticle(title: string, text: string): {
    ...
    // Normalize to 0-100
    const cleRelevance = Math.min(100, relevanceScore);

    return {
        categories: matchedCategories.length > 0 ? matchedCategories : ['general'],
        cleRelevance: 100, // Operator curated = always max relevance
    };
}
