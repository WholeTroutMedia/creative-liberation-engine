...
function translatePath(p: string): string {
    if (process.platform === 'win32' && p.startsWith('/volume2/')) {
        if (p.startsWith('/app/')) {
            return p.replace('/app/', '\\\\127.0.0.1\\docker\\').replace(/\//g, '\\');
        }
        return p.replace('/volume2/', '\\\\127.0.0.1\\').replace(/\//g, '\\');
    }
    return p;
}
...
export const CONFIG = {
    ...
    // On Windows: UNC path \\127.0.0.1\docker\genesis-deploy\runtime
    nasRoot: translatePath(process.env.NAS_RUNTIME_PATH || (process.env.NODE_ENV === 'production'
    ...
    // Obsidian Vault
    obsidianVaultPath: translatePath(process.env.OBSIDIAN_VAULT_PATH || (process.env.NODE_ENV === 'production'
    ...
    genkitBaseUrl: process.env.GENKIT_BASE_URL || process.env.GENKIT_URL || 'http://localhost:4100',
