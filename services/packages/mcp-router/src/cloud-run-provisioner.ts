...
export class CloudRunProvisioner {
  ...
  constructor(options?: {
    ...
    this.projectId  = options?.projectId  ?? process.env['GCP_PROJECT_ID']    ?? 'cle-engine-prod';
    this.defaultImage = options?.defaultImage ?? process.env['CLOUD_RUN_BASE_IMAGE'] ?? 'gcr.io/cle-engine-prod/cle-runtime:latest';
    this.dispatchUrl = options?.dispatchUrl  ?? process.env['DISPATCH_SERVER'] ?? 'http://127.0.0.1:5150';
  }
  ...
  async provisionTenant(spec: CloudRunServiceSpec): Promise<CloudRunServiceRecord> {
    ...
    if (this.projectId !== 'cle-engine-local') {
