...
export const modelScorecard = new ModelScorecard([
  {
    model_id: 'gemma-4-31b-dense:latest',
    provider: 'local',
    capabilities: { reasoning: 'frontier', speed: 'standard', context_size: 'xlarge', modality: ['text', 'image'], privacy: 'local_only' },
    cost_per_1k_tokens: 0, latency_p50: 1500, latency_p99: 3000,
    quality_score: 95, quality_dimensions: { reasoning_accuracy: 94, creative_quality: 90, instruction_following: 95, code_generation: 92, factual_accuracy: 93, speed_score: 80 },
    last_benchmarked: new Date().toISOString(), status: 'active', uptime_30d: 100, is_local: true
  },
  {
    model_id: 'gemma-4-e2b:latest',
    provider: 'local',
    capabilities: { reasoning: 'efficient', speed: 'realtime', context_size: 'large', modality: ['text', 'image', 'audio'], privacy: 'local_only' },
    cost_per_1k_tokens: 0, latency_p50: 200, latency_p99: 500,
    quality_score: 85, quality_dimensions: { reasoning_accuracy: 80, creative_quality: 85, instruction_following: 90, code_generation: 85, factual_accuracy: 85, speed_score: 99 },
    last_benchmarked: new Date().toISOString(), status: 'active', uptime_30d: 100, is_local: true
  },
  {
    model_id: 'deepseek-v3.2:latest',
    provider: 'local',
    capabilities: { reasoning: 'frontier', speed: 'fast', context_size: 'xlarge', modality: ['text', 'code'], privacy: 'local_only' },
    cost_per_1k_tokens: 0, latency_p50: 800, latency_p99: 2000,
    quality_score: 96, quality_dimensions: { reasoning_accuracy: 97, creative_quality: 85, instruction_following: 94, code_generation: 98, factual_accuracy: 95, speed_score: 90 },
    last_benchmarked: new Date().toISOString(), status: 'active', uptime_30d: 100, is_local: true
  }
]);