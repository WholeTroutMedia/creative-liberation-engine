...
def _set_output_sink(
    text_result: str, sink: llmfn_outputs.LLMFnOutputsSink
) -> None:
  sink.write_outputs(
                  ...
                  output_rows=[
                      llmfn_output_row.LLMFnOutputRow(
                          data={
                              ...
                              llmfn_outputs.ColumnNames.TEXT_RESULT: (
                                  text_result
                              ),
                          },
                          result_type=str,
...
class CmdLineParserRunTest(CmdLineParserTestBase):
  ...
  def test_parse_input_and_output_args(self):
    ...
    self.assertEqual(
        "one",
        _OUTPUT_VAR_ONE[0].output_rows[0][
            llmfn_outputs.ColumnNames.TEXT_RESULT
        ],
    )
    ...
    self.assertEqual(
        "two",
        _OUTPUT_VAR_TWO[0].output_rows[0][
            llmfn_outputs.ColumnNames.TEXT_RESULT
        ],
    )
