#!/usr/bin/env python3
"""
CORTEX AI Developer Portals Harvester
=====================================
Automated Playwright ingestion for public AI developer portals.
Extracts Anthropic prompt engineering & computer use guides, and OpenAI structured outputs guidelines,
...
# NAS Configuration
NAS_RAG_DATA = r"\\127.0.0.1\docker\creative-liberation-engine\media_intake\Sovereign_Academy_RAG\AIDeveloper"
ACADEMY_CODEX_DIR = r"y:\creative-liberation-engine\academy\codex\ai-developer"
...
def dispatch_ingestion_task(title: str, file_path: str, dry_run: bool = False):
    ...
    task_payload = {
        ...
        "type": "ai_developer_ingest",
        "priority": "high",
        "status": "pending",
        "payload": {
            "platform": "AI Developer Portal",
            "title": title,
            "rag_path": file_path,
...
def write_obsidian_note(title: str, url: str, summary: str, raw_content: str, dry_run: bool = False):
    ...
    memory_id = f"mem_ai_dev_{safe_title}"
...
source: "KI"
provenance:
  recordedBy: "ai_developer_harvester"
...
async def run_harvester(targets: dict, limit: int = 5, dry_run: bool = False):
    print("\n" + "#"*60)
    print("  CORTEX AI DEVELOPER PORTALS LEARNING HARVESTER")
    print("#"*60 + "\n")
        ...
        for name, url in targets.items():
            ...
            try:
                ...
                if limit > 1:
                    ...
                    links = await page.locator("a[href*='/docs/'], a[href*='/guides/']").all_hrefs()
                    # Deduplicate and filter absolute URLs
                    sub_urls = []
                    for l in links:
                        full_url = l if l.startswith("http") else f"https://docs.anthropic.com{l}" if "anthropic" in url else f"https://platform.openai.com{l}"
    ...
    print("\n" + "="*60)
    print("  AI DEVELOPER PORTALS LEARNING HARVEST SWEEP COMPLETE")
    print("="*60)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AI Developer Portals Harvester")
    parser.add_argument("--url", help="Target a specific URL instead of defaults")
