...
* **Issuer:** \`https://inception.engine/api/auth-md\`
...
app.get("/api/auth-md/health", (req, res) => {
  res.json({ status: "OK", service: "auth-md-hub-daemon" });
});
...
app.post("/api/auth-md/register", (req: any, res: any) => {
...
app.post("/api/auth-md/verify", (req: any, res: any) => {
...
if (process.env.NODE_ENV !== "test") {
  app.listen(PORT, () => {
    logger.info(`[AUTH-MD] Auth-Metadata Hub active on port ${PORT}`);
  });
}
