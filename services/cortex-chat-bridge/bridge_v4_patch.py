#!/usr/bin/env python3
"""CORTEX Chat Bridge V4 â€” Fixed Navigation + Genkit LLM Responses.
...
GENKIT_URL = os.environ.get("GENKIT_URL", "http://127.0.0.1:5170")
...
# Track messages we've already processed (hash â†’ timestamp)
...
def resolve_cdp_url(url: str) -> str:
    ...
    try:
        ...
        log.info(f"Resolved CDP URL: {url} â†’ {resolved}")
        return resolved
...
async def connect_cdp(pw) -> Browser:
    ...
    for attempt in range(20):
        try:
            ...
            log.info(f"âœ“ Connected to CDP at {cdp_endpoint}")
            return browser
        except Exception as e:
...
async def find_or_open_chat(browser: Browser) -> Page:
        ...
        for page in ctx.pages:
            try:
                if "chat.google.com" in page.url:
                    log.info(f"âœ“ Found existing Chat tab: {page.url}")
    ...
    if "chat.google.com" in url:
        log.info(f"âœ“ Google Chat opened: {url}")
        return page
    elif "accounts.google.com" in url:
        log.warning("Session expired â€” attempting automated re-login...")
...
async def auto_login(page: Page) -> bool:
    try:
        ...
        if "accountchooser" in page.url:
            log.info("Account chooser detected â€” clicking CORTEX account...")
            try:
        ...
        if "Verify it" in content or "captcha" in content.lower():
            log.error("Google requires verification â€” manual login needed via noVNC")
            return False

        log.warning(f"Post-login URL: {page.url} â€” checking if usable...")
        await page.goto(CHAT_URL, wait_until="domcontentloaded", timeout=30000)
        await asyncio.sleep(3)
    ...
    try:
        dismissed = await page.evaluate('''() => {
            ...
            for (const btn of buttons) {
                ...
                if (text === 'Disable' || text === 'No thanks' || text === 'Dismiss' || text === 'Got it') {
...
async def navigate_to_dm(page: Page) -> bool:
    ...
    This is the CRITICAL fix â€” V3 never called this function, so the bridge
...
async def get_visible_messages_js(page: Page, count: int = 15) -> list[dict]:
    """Extract messages using pure JS evaluation â€” most reliable approach.
    ...
    try:
        messages = await page.evaluate('''(maxCount) => {
            ...
            // Strategy 4: Broadest â€” any element with role=listitem inside the content area
            if (!groups.length) {
            ...
            // Strategy 5: Even broader â€” look for common message container patterns
            if (!groups.length) {
...
def get_ai_response(message_text: str, sender: str = "unknown") -> str:
    ...
    try:
        ...
        if resp.ok:
            ...
            if reply:
                log.info(f"â† Genkit response: {reply[:80]}...")
                return reply
    except Exception as e:
...
def forward_to_dispatch(message_text: str, sender: str = "unknown") -> str:
    ...
    try:
        log.info(f"â†’ Dispatch task: {message_text[:80]}...")
        resp = requests.post(
        ...
        log.info(f"â† Dispatch: Task created â†’ {task_id}")
...
async def send_chat_message(page: Page, text: str) -> bool:
    ...
    try:
        ...
        log.info(f"âœ“ Sent response ({len(send_text)} chars)")
        return True
    except Exception as e:
...
async def poll_loop(page: Page):
    """Main polling loop â€” navigates to DM, watches for new messages, responds."""
        ...
        try:
            ...
            if "chat.google.com" not in page.url:
                ...
                if "accounts.google.com" in page.url:
                    log.warning("Session expired â€” attempting auto-login...")
            ...
            if poll_count % 5 == 0:
                try:
                    with open("/app/data/last_messages_debug.json", "w") as f:
                        import json
                        json.dump(messages, f, indent=2)
                except Exception as e:
                    log.error(f"Failed to dump debug messages: {e}")
            ...
            for msg in messages:
                ...
                new_count += 1
                log.info(f"ðŸ“¨ New message from {sender}: {text[:80]}...")
...
async def main():
    """Entry point â€” connect via CDP â†’ find Chat â†’ navigate to DM â†’ poll."""
    log.info("=" * 60)
    log.info("CORTEX Chat Bridge V4 â€” Fixed Navigation Edition")
        ...
        try:
            async with async_playwright() as pw:
                ...
                if not await check_logged_in(page):
                    log.warning("Not logged in â€” attempting auto-login...")
                    if not await auto_login(page):
                        log.error(
                ...
                log.info("âœ“ CORTEX is logged into Google Chat")
                await dismiss_popups(page)
