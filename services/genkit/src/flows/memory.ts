 ...
 *
 * Implements the VERA SCRIBE mode memory extraction pattern
...
// ─────────────────────────────────────────────────────────────────────────────
// PATTERN EXTRACTION — SCRIBE (VERA's memory mode)
// ─────────────────────────────────────────────────────────────────────────────

async function extractPattern(task: string, outcome: string): Promise<string> {
    try {
        const { text } = await ai.generate(await withHumanState({
            model: process.env.GENKIT_DEFAULT_MODEL || 'googleai/gemini-2.5-flash',
            prompt: `You are VERA's SCRIBE mode. Extract a single reusable principle ("The Why") from this completed task.
