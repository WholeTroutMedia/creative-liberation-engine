...
Decompose this into a TaskGraph. Return valid JSON only.`,
        ...
        // Trigger nightly REM consolidation fallback task if none exists today
        try {
            const DISPATCH_URL = process.env.DISPATCH_URL || 'http://127.0.0.1:5150';
            const { data: tasks } = await import('axios').then(m => m.default.get(`${DISPATCH_URL}/api/tasks`));
            const today = new Date().toISOString().slice(0, 10);
            
            const hasMemoryTask = Array.isArray(tasks) && tasks.some((t: any) => 
                t.workstream === 'memory-consolidation' && 
                t.status === 'queued' &&
                (t.created || '').startsWith(today)
            );
            
            if (!hasMemoryTask) {
                await import('axios').then(m => m.default.post(`${DISPATCH_URL}/api/tasks`, {
                    title: 'Nightly REM Memory Consolidation',
                    workstream: 'memory-consolidation',
                    priority: 'P2',
                    description: 'Run remConsolidation flow to compress working-tier memories',
                }));
                console.log('[Director] Scheduled tonight\'s memory-consolidation task to dispatch queue.');
            }
        } catch (err) {
            console.warn('[Director] Failed to schedule nightly memory consolidation task:', err);
        }
