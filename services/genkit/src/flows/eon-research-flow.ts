 ...
 *                     into a SCRIBE-ready intelligence brief
...
const EonResearchInputSchema = z.object({
  ...
  /** If true, output is formatted for SCRIBE memory write */
...
const EonResearchOutputSchema = z.object({
  ...
  cle_engine_opportunities: z.array(z.object({
...
CLE ENGINE CONTEXT:
- 92-package TypeScript monorepo: sovereign agentic OS built by Sovereign Artist
...
Analyze "${input.topic}" deeply and return structured intelligence.${input.scribe_format ? '\n\nFormat output for insertion into SCRIBE persistent memory — include rich scribe_tags for future retrieval.' : ''}`,
...
SENDER: Sovereign Artist / Creative Liberation Engine (Creative Liberation Engine v5)
...
CLE ENGINE KEY FACTS (use selectively, don't dump all):
...
- Artist's background: operator building the intersection of AI orchestration + creative liberation
...
- send_ready: true if this email is good to send as-is, false if it needs Artist's personal voice added
