...
import fs from 'fs';
import { resolveHandoffMdPath } from './repo-root.js';
import { shouldInvokeKeeperRecall } from './keeper-recall-router.js';
...
function parseHandoff(): HandoffContext | null {
    try {
        const content = fs.readFileSync(resolveHandoffMdPath(), 'utf-8');
...
export async function keeperBootRecall(options: {
    ...
    const hadHandoffFile = fs.existsSync(resolveHandoffMdPath());
    if (!shouldInvokeKeeperRecall({ taskHint: taskContext, hadHandoffFile })) {
        const lines: string[] = [
            `## KEEPER v2 Memory Brief — ${new Date().toISOString()}`,
            `**Task context:** ${taskContext}`,
            '**Vector recall:** skipped (no meaningful task signal — see `docs/MEMORY_CONTRACT.md`).',
            '',
        ];
        if (handoff) {
            lines.push(
