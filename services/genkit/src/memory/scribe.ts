...
export const ScribeRecallInputSchema = z.object({
  ...
  category: z.union([MemoryCategory, z.array(MemoryCategory)]).optional().describe('Filter by memory category (or list of categories)'),
...
export const scribeRecall = ai.defineTool(
  ...
  async (input): Promise<z.infer<typeof ScribeRecallOutputSchema>> => {
    ...
    if (input.tags && input.tags.length > 0) where['tags'] = { '$in': input.tags };
    ...
    const filtered = rawResults.filter((entry: any) => {
      ...
      if (input.category) {
        if (Array.isArray(input.category)) {
          if (!input.category.includes(entryCategory)) return false;
        } else {
          if (entryCategory !== input.category) return false;
        }
      }
      if (input.tags && input.tags.length > 0 && !input.tags.some(t => entry.tags?.includes(t))) return false;
      return true;
    });

    return {
      results: filtered.map((entry: any) => {
        const meta = (entry as any).metadata ?? {};
        return {
