 ...
 */
function loadThemeRegistry(): RegistryData {
 ...
 */
export function routeManifestToThemes(manifest: JobManifest): { primaryTheme: ThemeDefinition | null; score: number } {
    ...
    for (const theme of registry.themes) {
        ...
        const hasDirectCategoryMatch = manifest.categories.some(c => 
            themeKeywords.has(c.toLowerCase()) || theme.name.toLowerCase().includes(c.toLowerCase())
        );
 ...
 */
export function linkJobToThemeFile(manifest: JobManifest, theme: ThemeDefinition): void {
    ...
    const obsidianLink = `file:///app/runtime/nexus-vault/Sentinel/${manifest.filename}.md`;
