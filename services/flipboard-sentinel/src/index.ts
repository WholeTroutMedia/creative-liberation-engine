...
import { pollRSSFeed, checkFeedHealth, type FeedArticle } from './rss-poller.js';
...
async function processArticle(article: FeedArticle): Promise<JobManifest | null> {
    ...
    // Step 2: Classify
    const { categories, inceptionRelevance } = classifyArticle(article.title, fullText);
    console.log(`[SENTINEL] ðŸ“Š Classification: [${categories.join(', ')}] Relevance: ${inceptionRelevance}`);
    ...
    const manifest = createManifest(sourceArticle, categories, inceptionRelevance);
...
async function runPollCycle(): Promise<void> {
    ...
    try {
        // Health check
        const healthErr = await checkFeedHealth();
        ...
        // Poll RSS
        const allArticles = await pollRSSFeed();
