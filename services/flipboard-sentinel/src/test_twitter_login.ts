...
async function main() {
    ...
    const versionRes = await fetch('http://122.0.3.1:9224/json/version');
    ...
    try {
        ...
        const email = process.env.TWITTER_EMAIL || 'inquiries@inceptionengine.systems';
        ...
        if (usernameExists) {
            ...
            if (suspiciousExists) {
                ...
                const phoneOrUser = process.env.TWITTER_USERNAME || 'InceptionEngine';
