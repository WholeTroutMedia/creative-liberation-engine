#!/usr/bin/env pwsh
# genkit-autostart.ps1 Ã¢â‚¬â€ Boot Genkit engine on logon
# Probes NAS first, then local. Starts local if both unreachable.

$log = Join-Path $PSScriptRoot "genkit-autostart.log"
$nasUrl = "http://127.0.0.1:4100/health"
$localUrl = "http://localhost:4100/health"
$engineDir = Join-Path (Split-Path $PSScriptRoot -Parent) "packages\genkit"
...
if (Test-Endpoint $nasUrl) {
    WL "NAS Genkit online Ã¢â‚¬â€ no action needed"
    exit 0
}
...
if (Test-Endpoint $localUrl) {
    WL "Local Genkit already running Ã¢â‚¬â€ no action needed"
    exit 0
}

WL "Local Genkit offline Ã¢â‚¬â€ starting..."
