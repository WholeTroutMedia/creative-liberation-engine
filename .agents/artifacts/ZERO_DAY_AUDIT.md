...
| `packages/cle-agent-extension/public/manifest.json` | `host_permissions: ["http://127.0.0.1:5050/*"]` | **FIX:** Build-time env injects public dispatch URL; or add `https://*dispatch*/*` and read URL from storage. |
| `packages/cle-agent-extension/src/popup.ts` | `DISPATCH_BASE = 'http://127.0.0.1:5050'` | **FIX:** From storage or build env (e.g. `import.meta.env.VITE_DISPATCH_URL`). |
| `packages/cle-agent-extension/src/background.ts` | `DISPATCH_BASE = 'http://127.0.0.1:5050'` | **FIX:** Same. |
| `packages/cle-agent-extension/dist-firefox/background.js` | Hardcoded 192.168 (built artifact) | Rebuild with env; do not commit built dist with private IP. |
| `packages/cle-browser/src/dispatch-heartbeat.ts` | `DISPATCH_URL = 'http://127.0.0.1:5050'` | **FIX:** `process.env.DISPATCH_URL ?? 'http://localhost:5050'` (server-side only; client never runs this). |
| `packages/cle-browser/src/intelligence/action-planner.ts` | `GENKIT_NAS_URL = "http://127.0.0.1:4100"` | **FIX:** `process.env.GENKIT_URL ?? 'http://localhost:4100'`. |
...
| `ecosystem/creative-liberation-engine/apps/console` | DispatchCenter.tsx hardcodes 127.0.0.1:5050 | **FIX:** Env-based; sync from creative-liberation-engine-v5. |
...
| `packages/mcp-servers/cle-dispatch` | IE_DISPATCH_API 127.0.0.1:4800 | Same. |
| `packages/mcp-servers/cle-registry` | IE_DISPATCH_URL 127.0.0.1:5050 | Same. |
...
grep -r "192\.168" apps/console/dist packages/cle-agent-extension/dist* 2>/dev/null && echo "FAIL: 192.168 in client bundle" || echo "PASS"
...
- **packages/cle-browser** â€” dispatch-heartbeat, action-planner, vera-bridge, vision-model: DISPATCH_URL / GENKIT_URL from env; no 192.168.
- **packages/cle-agent-extension** â€” manifest.json & manifest.firefox.json: host_permissions use localhost + https patterns; popup/background use localhost defaults; Open Console link â†’ localhost:3000. **Rebuild dist-firefox** so built artifacts no longer contain 192.168.
