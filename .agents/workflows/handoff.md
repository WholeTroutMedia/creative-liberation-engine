...
3.5. **Background Task Auto-Cleanup:**
   - Query all running background tasks using the `manage_task` tool with action `list`.
   - Identify any tasks that have completed, failed, or are orphaned/stale.
   - For all identified inactive or unnecessary tasks, invoke `manage_task` with action `kill` using their `TaskId` to release system resources and prevent metadata/zombie accumulation in the registry.
