...
- **Ease of use:** Do not ask the user to pick design tools or MCPs. Retrieve anchors and run the workflow **silently**; the user approves **visual outcomes** and product fit, then we **bias to PLAN/SHIP** (Zero Day).
...
### 2. Live Web Discovery & Trend Research

Before compiling prompts:
- Run targeted web searches (e.g. Awwwards, Refero, YouTube) matching the task's context (e.g., `'site:refero.design UI patterns [task context]'`) to extract current trending layouts and CSS structure.
- Check `docs/design-system/pattern-index.md` and `tools/design-library/` for the 3 closest local patterns.
- Synthesize a custom, task-specific design playbook. **Banned Presets:** Never copy-paste or reuse static presets (such as CoTrain, Sentinel, Void, Tactile, Architectural).
...
Ensure the compiled prompts are completely custom and represent the specific functional context of the active task.
