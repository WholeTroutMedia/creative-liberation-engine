...
```powershell
// turbo
Get-Content -Path "Y:\\creative-liberation-engine\.agents\scribe\training_corpus\baseline.jsonl" | Measure-Object -Line
```
...
docker run --gpus '"device=' + $targetDevice + '"' -v "Y:\\creative-liberation-engine\.agents\scribe\training_corpus:/workspace/data" -v "Y:\\creative-liberation-engine\.agents\models:/workspace/models" -it unsloth/unsloth:latest bash -c "python train.py --data_path /workspace/data/baseline.jsonl --output_dir /workspace/models/archaeon-v1 --model_name unsloth/llama-3-8b-Instruct-bnb-4bit"
```
