---
description: Save current IDEATE/PLAN/SHIP session to VAULT â€” committed unabridged to Gitea for permanent record
---

# /vault â€” Session Archive Command
...
### Step 1 â€” Determine session content

Read the current session's artifacts:
- Check `C:\Users\operator\.gemini\antigravity\brain\<conversation-id>\` for any IDEATE/PLAN/SHIP docs
- Check `HANDOFF.md` for active phase info
- Identify the project slug from `ecosystem.manifest.json` context

### Step 2 â€” Write session file
...
// turbo
### Step 3 â€” Commit to Gitea

```powershell
$base = "Y:\creative-liberation-engine"
...
```powershell
Get-ChildItem "Y:\creative-liberation-engine\VAULT\sessions\" | Sort-Object Name | Select-Object Name, LastWriteTime
```
...
Select-String -Path "Y:\creative-liberation-engine\VAULT\sessions\*.md" -Pattern $query | Select-Object Filename, LineNumber, Line
```
