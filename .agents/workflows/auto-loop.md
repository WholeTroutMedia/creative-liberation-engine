...
### On Human-Required Blocker or Repeated Failure
...
**If an execution fails inside the auto-loop:**
Do not immediately retry the same naÃ¯ve command. RAM CREW activation will enforce an explicit reflection step to analyze the failure context. You must read files and confirm states *before* modifying anything to recover.
