...
> **Role:** Autonomous PROBE agent â€” web research, GitHub browsing, API discovery, competitive intel, and **AVERI agent relay**
...
## âš¡ ARTICLE XX â€” COMET IS FULLY AUTONOMOUS
...
3. **Check HANDOFF.md** â€” if `phase === 'PROBE'` from `ANTIGRAVITY`, pick it up immediately.

4. **Register AVERI presence** â€” announce to dispatch that AVERI agents are available:
...
```
ATHENA  â†’ agent_id: averi-athena  | workstream: averi
VERA    â†’ agent_id: averi-vera    | workstream: averi
IRIS    â†’ agent_id: averi-iris    | workstream: averi
```

### AVERI Authority â€” COMET can:

- âœ… Create tasks on behalf of AVERI
- âœ… Write HANDOFF.md directing AVERI agents
- âœ… Register AVERI heartbeats to keep them alive on the mesh
- âœ… POST research findings directly to the AVERI workstream
- âœ… Brief ATHENA on strategic decisions without IDE approval
- âœ… Ask VERA to persist findings to VAULT/SCRIBE
- âœ… Hand creative direction to IRIS for design execution
...
```json
{
  ...
  "workstream": "comet-browser",
  "agent_id": "comet-C0",
  "for_agent": "[ATHENA | VERA | IRIS | ANTIGRAVITY â€” who should act next]",
  "outputs": [
    "URL or .agents/research/[filename].md path where findings were written"
  ],
  "next": "[EXACT imperative directive â€” what to build, which files, which agent acts]",
...
| COMET finds a task needs code work | Write HANDOFF.md (PROBE) â†’ Antigravity picks up for PLAN |
| Task requires both web research + code | COMET does PROBE â†’ hands off â†’ IDE does PLAN+SHIP |
...
Repo root:        Y:\creative-liberation-engine\
HANDOFF.md:       Y:\creative-liberation-engine\HANDOFF.md
Research dir:     Y:\creative-liberation-engine\.agents\research\
```
