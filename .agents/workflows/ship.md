---
description: Activate SHIP mode Ã¢â‚¬â€ PRISM + builders implement a PLAN spec with full type safety, Article IX quality standards, and auto dispatch resolution
---
...
// turbo
2. **Sync** Ã¢â‚¬â€ pull latest before touching any files:

   ```powershell
   git -C "Y:\\creative-liberation-engine" pull origin main --rebase
   ```

1. **Check dispatch registry** Ã¢â‚¬â€ read `.agents/dispatch/registry.md`. Confirm no other active window owns the files you're about to touch. If conflict: stop, leave a note in registry.md, pick a different task.
...
3. **Implement Ã¢â‚¬â€ file by file:**
   - Complete implementation only Ã¢â‚¬â€ no stubs, no `// TODO`, no `any` types
...
4. **Type check** Ã¢â‚¬â€ must be zero errors before continuing:
...
5. **Run tests** Ã¢â‚¬â€ if tests exist for affected package:

   ```powershell
   pnpm --filter @cle/<package> test
   ```

   If no tests exist for new code, write them. Article IX: no untested new logic.

6. **Update dispatch** Ã¢â‚¬â€ mark task done:
   - Update `.agents/dispatch/task-queue.md` Ã¢â‚¬â€ move task to Completed table
   - Update `.agents/dispatch/registry.md` Ã¢â‚¬â€ update last-seen timestamp

// turbo
9. **Write HANDOFF.md** Ã¢â‚¬â€ pass the baton to VALIDATE:
...
1. **Offer release pipeline:**
    ...
    ```
    Ã¢Å“â€¦ SHIP complete Ã¢â‚¬â€ <topic>

    Ready to:
    1. `/validate` Ã¢â‚¬â€ LOGD + NORTHSTAR full quality gate (recommended)
    2. `/release` Ã¢â‚¬â€ commit + PR + deploy in one command
    3. `/pr` Ã¢â‚¬â€ open a PR for human review
...
// Ã¢ÂÅ’ Never
const x: any = ...           // no any Ã¢â‚¬â€ use unknown + type narrowing
// @ts-ignore                // no suppression Ã¢â‚¬â€ fix the type
```
...
| File owned by another window | Stop Ã¢â‚¬â€ note in registry.md, pick different task |
...
- Strict mode is ON Ã¢â‚¬â€ `tsconfig.json` has `"strict": true` across all packages
