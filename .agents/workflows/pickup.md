---
description: Pick up the next available task from the CLE Dispatch Server Ã¢â‚¬â€ no workspace, no slash commands, no configuration needed
---

# pickup (natural language Ã¢â‚¬â€ trigger phrases)
...
2. **Use the smart next-task endpoint** (server picks the best match for this agent):
   ...
   Filter: exclude workstreams already `active` in another agent. Sort P0Ã¢â€ â€™P3, oldest first.
...
// turbo
5. Run `git -C "Y:\\creative-liberation-engine" pull origin main --rebase` silently.

1. Announce and begin:

   ```
   Ã¢Å“â€¦ Window [X] Ã¢â‚¬â€ Claimed [workstream]
   Ã°Å¸â€œâ€¹ [Task ID]: [title]
   Priority: [P#]
...
2. **On task completion Ã¢â‚¬â€ auto-continue** (if `/auto-loop` is active or user said "keep going"):
...
Report: "Ã°Å¸ËœÂ´ Queue empty. Add a task with `add_task`, tell me what to work on, or I'll standby."

## Rules

- Project root is always `Y:\creative-liberation-engine` Ã¢â‚¬â€ never ask the user for it
- Dispatch server is always at `http://127.0.0.1:5050` Ã¢â‚¬â€ no configuration needed
- Never claim a workstream already held by an active agent in the status response
- Always check blockers before claiming Ã¢â‚¬â€ P0/P1 blockers take priority over new task claims
