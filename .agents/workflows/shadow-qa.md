---
description: Shadow QA agent Ã¢â‚¬â€ automatically runs tests + browser validation after every code change
---
...
1. **Surgical Mode** Ã¢â‚¬â€ triggered after every approved change
2. **Manual** Ã¢â‚¬â€ use `/qa` slash command or say "run shadow QA"

## Step-by-Step

### Step 1: Identify Affected Package
From the changed file path, determine the package name:
- `packages/genkit/**` Ã¢â€ â€™ `@cle/genkit`
- `packages/genmedia/**` Ã¢â€ â€™ `@cle/genmedia`
- `apps/console/**` Ã¢â€ â€™ `console` (React app)
- `packages/finance-agent/**` Ã¢â€ â€™ `@cle/finance-agent`
- etc.

// turbo
### Step 2: Run TypeScript Check
```powershell
cd "Y:\\creative-liberation-engine"
pnpm --filter <package-name> build 2>&1 | tail -20
```
- PASS Ã¢â€ â€™ continue to Step 3
- FAIL Ã¢â€ â€™ enter fix loop (show error, propose fix, apply, re-run)
...
- POST to `http://localhost:6000/scan` (SPECTRE QA agent):
  ...
  ```
- If SPECTRE is not running: skip and note "[QA] SPECTRE unavailable Ã¢â‚¬â€ manual visual check recommended"

### Step 5: Report Summary
Output a compact QA summary:
```
[SHADOW QA] Ã¢Å“â€œ TypeScript: PASS
[SHADOW QA] Ã¢Å“â€œ Tests: 12 passed, 0 failed
[SHADOW QA] Ã¢Å“â€œ Visual: SPECTRE scan clean
[SHADOW QA] Ready for next change.
```

Or on failure:
```
[SHADOW QA] Ã¢Å“â€” TypeScript: FAIL Ã¢â‚¬â€ see errors below
