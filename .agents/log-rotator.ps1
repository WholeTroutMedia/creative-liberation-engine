#!/usr/bin/env pwsh
# log-rotator.ps1 Ã¢â‚¬â€ Cap all .agents/ logs at 500 lines
...
Get-ChildItem $agentsDir -Filter "*.log" | ForEach-Object {
    ...
    if ($content -and $content.Count -gt $maxLines) {
        ...
        $header = "# [rotated $(Get-Date -Format 'yyyy-MM-dd HH:mm') Ã¢â‚¬â€ kept last $maxLines lines]"
        @($header) + $kept | Set-Content $file -Encoding UTF8
        Write-Host "Rotated: $($_.Name) ($($content.Count) Ã¢â€ â€™ $maxLines lines)"
    }
}
