...
> [!IMPORTANT]
> **NO STATIC PRESETS OR TEMPLATES:**
> Traditional design presets (such as Swiss Brutalism, CoTrain, Sentinel, Void, Tactile, Architectural) are deprecated. Creative expression must be synthesized dynamically from first principles based on the active task's unique functional context. Never copy-paste predefined layout descriptors, color schemes, or typography pairings.
>
> **TASK-SPECIFIC VISUAL CONTEXTUALIZATION:**
> When generating visual mockups (`generate_image`), the visual design MUST directly represent the actual interface components, data layouts, buttons, and content described in the user's active task (e.g. if the task is an audio visualizer, the mock must depict waveforms and play controls; if a database viewer, it must show table columns and query feeds). Never display a generic, unrelated dashboard wireframe or placeholder.
...
### Step 1 — Live Web Discovery & ATELIER Retrieval

Before compiling prompts, perform targeted research to discover contemporary design trends:

- **Targeted Live Search:** Query the live web (e.g. Awwwards, Refero, YouTube, Dribbble, Twitter/X) using specific search terms for the category of application being designed (e.g., `'site:refero.design modern music app UI patterns'`, `'site:godly.website audio player layout'`) to extract structural CSS, kinetic motion values, and typographic rules.
- **ATELIER Local Alignment:** Check `docs/design-system/pattern-index.md` and the relevant `tools/design-library/<source>/README.md` (or `catalog.json`) files for local patterns closest to the use case.
- **Synthesize Visual Playbook:** Compare discovered trends with local rules to synthesize a unique, custom visual playbook. Do not reuse static template playbooks. These anchors will be embedded in the DESIGN_CONTRACT under "Component Anchors".
