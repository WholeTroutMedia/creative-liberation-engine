...
# SCRIBE Memory â€” Creative Liberation Engine v5
...
- A bug is discovered and fixed â€” document the root cause and fix
...
- Starting IDEATE â€” retrieve prior sessions on this topic
- Starting PLAN â€” retrieve relevant architecture decisions and conventions
- Debugging an issue â€” retrieve prior bug fixes in this area
- User asks "what did we decide aboutâ€¦" or "what's our pattern forâ€¦"
...
|---------|------|---------|
| **Episodic** | "What happened" â€” session logs, decisions, events | ChromaDB `episodic` collection |
| **Semantic** | "How things work" â€” patterns, conventions, architecture | ChromaDB `semantic` collection |
| **KI (Knowledge Item)** | "What we know" â€” distilled, long-term reference | Creative Liberation Engine KI system (see `docs/MEMORY_CONTRACT.md` â€” postcard vs newspaper) |
...
```powershell
# Search the SCRIBE archive in v5
Get-ChildItem "Y:\creative-liberation-engine\packages\memory\scribe_archive\" -Recurse -Filter "*.md" | Select-String "<query>"
```
...
### Step 2 â€” Write the Memory Entry
...
**What happened:** [2â€“3 sentence summary of the session/event]
...
### Step 3 â€” Choose the Write Target
...
**Fallback (engine offline) â€” write to local SCRIBE archive:**
```powershell
# Semantic memories
$path = "Y:\creative-liberation-engine\packages\memory\scribe_archive\semantic\"

# Episodic memories
$path = "Y:\creative-liberation-engine\packages\memory\scribe_archive\episodic\"
```
...
1. **New Genkit flow added** â†’ Write to semantic: flow name, input/output schema, purpose
2. **New package created** â†’ Write to semantic: package name, responsibility, key exports
3. **Database/schema change** â†’ Write to semantic: what changed and migration notes
4. **Performance optimization** â†’ Write to semantic: problem, solution, measured improvement
5. **Constitutional violation found and fixed** â†’ Write to episodic: what article, what the issue was, how it was resolved
6. **IDEATE direction selected** â†’ Write to episodic: topic, selected direction, rationale
7. **Design system decision** â†’ Write to semantic: component name, design token used, why
