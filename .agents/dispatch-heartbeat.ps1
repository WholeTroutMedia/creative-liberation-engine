#!/usr/bin/env pwsh
# dispatch-heartbeat.ps1 Ã¢â‚¬â€ Keep agent registrations alive in dispatch server
# Fires every 60s. Registers this machine as always-on ambient infrastructure.

$log = Join-Path $PSScriptRoot "dispatch-heartbeat.log"
...
while ($true) {
    ...
    $ok1 = Send-HB "cle-GIT"    "GIT"  "git-stream-monitor"  "autonomous git commit+push Ã¢â‚¬â€ cycle $cycle"
    # Keep ambient boot stack alive
    $ok2 = Send-HB "cle-BOOT"   "BOOT" "boot-stack"          "always-on boot stack monitoring"

    if ($ok1 -and $ok2) {
        WL "cycle $cycle Ã¢â‚¬â€ heartbeats OK"
    }
    else {
        WL "cycle $cycle Ã¢â‚¬â€ dispatch unreachable (NAS offline?)"
    }
