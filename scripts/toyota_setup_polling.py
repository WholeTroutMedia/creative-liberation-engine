...
OTP_FILE = "/app/creative-liberation-engine/runtime/session/otp_input.txt"

# Dynamic patch to support ui_locales and polling-based OTP input
                ...
                for cb in data["callbacks"]:
                    ...
                    if cb_type == "NameCallback":
                        ...
                        if "one time password" in prompt_val.lower() or "otp" in prompt_val.lower():
                            print(f"\n[SECURITY] {prompt_val}")
                            print(f"Waiting for OTP code to be written to {OTP_FILE}...")
                            ...
                            for _ in range(60):  # 120 seconds timeout
                                if os.path.exists(OTP_FILE):
                                    ...
                                    if otp_code:
                                        print(f"Successfully read OTP from file: {otp_code}")
                                        try:
                                            os.remove(OTP_FILE)
                                        except Exception:
                                            pass
                            ...
                            if not otp_code:
                                raise LoginError("Timeout waiting for OTP code input from user.")
...
async def main():
    ...
    # Ensure any stale OTP file is deleted
    if os.path.exists(OTP_FILE):
        try:
            os.remove(OTP_FILE)
        except Exception:
