...
Lineage: v4 start_engine.py + cle_engine/main.py → v5 server.py (unified)
"""
...
def _load_agents() -> None:
    ...
    try:
        from cle.agents.hives.broadcast.ram_crew import ram_crew
        _registry.register(ram_crew)
    except ImportError:
        logger.debug("RAM Crew agent not yet available")

    try:
        from cle.agents.hives.keeper.scribe import scribe
        _registry.register(scribe)
    except ImportError:
        logger.debug("SCRIBE agent not yet available")

    try:
        from cle.agents.hives.keeper.cosmos import cosmos
