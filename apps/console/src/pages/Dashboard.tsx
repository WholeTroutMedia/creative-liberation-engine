...
// ─── Constants (Zero Day: from env) ─────────────────────────────────────────────
const DISPATCH = DISPATCH_URL
const NAS_IP = DISPATCH.replace(/^https?:\/\/([^:/]+).*$/, '$1') // display only
...
/* ── DIRA-04: Creator Productivity types ─────────────────────── */
...
const FEATURES = [
    {
        ...
        badge: 'CREATE',
        badgeColor: '#F5A524',
        desc: 'Unified multimodal generation — image, video, music, voice, code — with 16+ tools and 150+ models via FAL, Vertex, Replicate.',
        route: '/nexus',
...
const STATIC_ACTIVITY = [
    { icon: '◎', label: 'Generated image', detail: 'Flux Pro · 1024×1024', time: '2m ago', color: '#F5A524' },
    { icon: '⚖', label: 'Constitution reviewed', detail: 'Article 7 · Agent access control', time: '8m ago', color: '#4285F4' },
    { icon: '⬢', label: 'Agent activated', detail: 'ATLAS · Broadcast Hive', time: '15m ago', color: '#FF6B35' },
    { icon: '⬡', label: 'Flow executed', detail: 'zero-day-intake · 3 steps', time: '1h ago', color: '#22c55e' },
    { icon: '◉', label: 'Scout session', detail: 'Browsed fal.ai models', time: '2h ago', color: '#9B72CF' },
]

/* ── Dispatch stat definitions ───────────────────────────────── */
...
function getGreeting(): string {
    ...
    if (h < 12) return 'Good morning, Custodian.'
    if (h < 17) return 'Good afternoon, Custodian.'
    return 'Good evening, Custodian.'
}
...
/* ── DIRA-04: Creator Productivity Panel ──────────────────────── */
        ...
        <section className="dira-panel" ref={cardRef}>
            <div className="dira-header">
                ...
                <div className="dira-header-right">
                    ...
                    <div className="dira-kpi">
                        <span className="dira-kpi-value" style={{ color: avgRate >= 90 ? '#22c55e' : avgRate >= 80 ? '#F5A524' : '#ef4444' }}>
                            {avgRate}%
                        </span>
            ...
            {expanded && (
                <div className="dira-expanded">
                    <div className="dira-section">
                        <div className="dira-section-title">Case Resolution Rate — 7 Day Rolling</div>
                                ...
                                <div key={i} className="dira-bar-col">
                                    ...
                                    <div className="dira-bar-track">
                                        <div
                                            ...
                                            style={{
                                                height: `${d.rate}%`,
                                                background: d.rate >= 90 ? '#22c55e' : d.rate >= 80 ? '#F5A524' : '#ef4444',
                                            }}
                                        />
                                    </div>
        ...
        <div className="dash-container">
            ...
            <div className="dash-hero">
                ...
                <div className="dash-hero-grid">
                    ...
                    {/* Recent activity — live from dispatch when online */}
                    <div className="dash-recent">
                        <div className="dash-recent-header">
                            <span className="dash-recent-label">RECENT ACTIVITY</span>
                            {dispatchOnline && <span className="dash-live-badge">● LIVE</span>}
                        </div>
            ...
            <div className="dash-hives">
                <div className="dash-hives-header">
                    ...
                    <button onClick={() => navigate('/agents')} className="dash-hives-link">View all →</button>
                </div>
            ...
            <div className="dash-board">
                <div className="dash-board-header">
                    ...
                    <span className={`dash-board-ip ${dispatchOnline ? 'dash-board-ip-ok' : 'dash-board-ip-err'}`}>
                        {dispatchOnline ? (DISPATCH.replace(/^https?:\/\//, '').replace(/\/.*$/, '')) : 'OFFLINE'}
                    </span>
                    {dispatch && (
                    ...
                    <button onClick={pollDispatch} className="dash-board-btn-refresh">↻</button>
                </div>
                ...
                {addingTask && (
                    <div className="dash-new-task">
                        ...
                        <select className="dash-input dash-select" aria-label="Task priority" value={newTask.priority} onChange={e => setNewTask(t => ({ ...t, priority: e.target.value }))}>
                ...
                {!dispatchOnline ? (
                    <div className="dash-offline-warn">
                        Dispatch server offline — run <code className="dash-offline-code">npm run dev --prefix packages/dispatch</code> to start it
                    </div>
                ) : dispatch && (
                    <div className="dash-queues">
                        <div className="dash-queue-panel">
                            ...
                            {!dispatch.queued_tasks.length && !dispatch.active_tasks.length && (
                                <div className="dash-empty">Queue empty — all caught up.</div>
                            )}
                        </div>
                        <div className="dash-queue-panel">
            ...
            <div className="dash-health">
                <div className="dash-health-header">
                    ...
                    <span className="dash-health-sub">· auto-refresh 10s{lastPolled ? ` · ${lastPolled.toLocaleTimeString()}` : ''}</span>
                    <button onClick={poll} className="dash-health-btn">↻ Refresh</button>
                </div>
