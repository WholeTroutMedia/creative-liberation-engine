...
export default defineConfig({
  ...
  server: {
    port: 5173,
    proxy: {
      '/api/genkit': { target: 'http://localhost:4000', rewrite: (p: string) => p.replace('/api/genkit', ''), changeOrigin: true },
      '/api/relay': { target: 'http://localhost:5000', rewrite: (p: string) => p.replace('/api/relay', ''), changeOrigin: true },
