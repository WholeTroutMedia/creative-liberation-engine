...
2. **Check for Dockerfile.**
   ```powershell
   Test-Path "D:\\Google Antigravity\\Infusion Engine Brainchild\\creative-liberation-engine\<service-dir>\Dockerfile"
   ```
...
3. **Verify build (dry-run check).**
   ...
   npx tsc --noEmit --project "D:\\Google Antigravity\\Infusion Engine Brainchild\\creative-liberation-engine\<service-dir>\tsconfig.json"
   ```
