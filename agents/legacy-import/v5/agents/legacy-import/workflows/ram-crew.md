...
**EXPLICIT REFLECTION REQUIREMENT (Mandatory — no recovery action without this):**
...
4. ROOT_CAUSE: [the specific mismatch between expected and actual — not the symptom]
...
> **Loop escape rule:** If you have attempted the same operation ≥ 3 times with the same failure, STOP. Do not retry a fourth time. Surface the completed FAILURE REFLECTION to the user.

// turbo
3. Identify root cause category using process diagnostics:

- **Terminal saturation** → Look for sustained High CPU (>=90%), Process state `D` (uninterruptible sleep), or stuck child processes.
- **Network timeout** → command waiting on unreachable host
- **Git lock** → `.git/index.lock` present — find what process holds the lock before removing it.
- **Ghost Port Lock** → `EADDRINUSE` error or frozen port caused by an orphaned background process
- **Process deadlock** → circular dependency in parallel commands
...
```powershell
Remove-Item "D:\\Google Antigravity\\Infusion Engine Brainchild\\creative-liberation-engine\.git\index.lock" -ErrorAction SilentlyContinue
```
