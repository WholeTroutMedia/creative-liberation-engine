...
- Assumes the user stores raw notes in `D:\Google Antigravity\Infusion Engine Brainchild\apps\braindump`
...
1. **Scan the Drop Zone**
   Check `D:\Google Antigravity\Infusion Engine Brainchild\apps\braindump` for newly added `.txt` or `.md` files. Exclude `v2-instructions.md` and `v2-master-knowledge.md` as they are foundational knowledge, not raw braindumps.
...
4. **Generate the Handoff Artifact**
   Write the processed UPE output into a new file in `D:\Google Antigravity\Infusion Engine Brainchild\apps\braindump\processed\`.
...
6. **Cleanup**
   Move the original raw file into `D:\Google Antigravity\Infusion Engine Brainchild\apps\braindump\archive\`.
