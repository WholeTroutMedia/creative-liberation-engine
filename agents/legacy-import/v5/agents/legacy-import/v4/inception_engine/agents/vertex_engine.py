...
class VertexAIEngine:
    ...
    async def generate_image(
        ...
        try:
            ...
            for idx, image in enumerate(response.images):
                ...
                images.append(
                    {"index": idx, "data": image_base64, "format": "png", "size": len(image_bytes)}
                )
    ...
    async def generate_text(
        ...
        if not self.initialized:
            return {"status": "error", "error": "Vertex AI not initialized", "cost_usd": 0}
        ...
        try:
            ...
            gemini_model = GenerativeModel(
                model, generation_config=generation_config, system_instruction=system_instruction
            )
