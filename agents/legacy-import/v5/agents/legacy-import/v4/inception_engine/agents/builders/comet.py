...
class COMETAgent(BaseAgent):
    ...
    def execute(self, context: Dict[str, Any]) -> AgentResult:
        ...
        Args:
            context: Execution context with mode, previous output, global rules
        ...
        try:
            ...
            if task_type == "api":
                output = self._build_api(task, context)
            elif task_type == "database":
                output = self._setup_database(task, context)
            elif task_type == "service":
                output = self._build_service(task, context)
            elif task_type == "integration":
                output = self._integrate_external(task, context)
            else:
                output = self._general_backend(task, context)
