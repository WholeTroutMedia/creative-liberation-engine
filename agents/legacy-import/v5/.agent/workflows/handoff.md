...
1. Update [registry.md](../dispatch/registry.md) — change your instance status to CLOSED, note the handoff:

```powershell
Write-Host "📝 Update .agent\dispatch\registry.md: set your row status to HANDED-OFF → $target" -ForegroundColor Yellow
```

1. Notify the receiving IDE window by updating [`project_dispatch.md`](../project_dispatch.md):

Open [`.agent/project_dispatch.md`](../project_dispatch.md) and update the Owner column for this slug to `[TARGET-INSTANCE] (pickup pending)`.

---

## Receiving a Handoff

When you’re the target instance, after running [`/dispatch`](./dispatch.md):

1. Look for `handoff-[slug].md` in [`.agent/dispatch/`](../dispatch)
2. Read the pickup note carefully
3. Run [`/claim [slug]`](./claim.md) to register yourself as the new owner
