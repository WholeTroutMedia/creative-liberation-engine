...
> Invoke-RestMethod "http://127.0.0.1:3000/api/v1/repos/Creative-Liberation-Engine/cle-engine/actions/workflows/deploy-genesis.yml/dispatches" -Method Post -Headers @{Authorization="token $token";"Content-Type"="application/json"} -Body '{"ref":"main"}'
> ```
>
...
### Operator ease (same spirit as Zero Day)

- **High-level intent is enough.** The agent breaks work into plain-language sections (what we’re building, first shippable slice, next step). The human must **not** be forced to name design tools, MCP servers, or ingest pipelines — unless they choose to.
- **Design is outcome-focused:** users react to screens and flows, not to internal toolchain menus. Move every project **toward immediate shipping** and final product; avoid extra back-and-forth whose only purpose is process.
...
|---------|----------|----------|
| Genkit | 127.0.0.1:4100 | Cloud Run `cle` us-central1 — **PRIVATE (Requires IAM)** |
| Dispatch | 127.0.0.1:5050 | Not public — needs Cloudflare Tunnel or proxy |
| CORE Mobile | NAS SMB + Firebase Hosting `cle-wtm.web.app` | **PRIVATE PREVIEW (Requires Auth)** |

### The CafÃ© Workflow (what we're building toward)

1. Jaymee opens CORE on iPhone from public URL
...
- Secure Genkit URL (Cloud Run with IAM/Auth)
...
- **Repo:** `Creative-Liberation-Engine/cle-engine`
...
- **Single source:** **creative-liberation-engine-v5** — Creative Liberation Engine GENESIS monorepo only.
- **Live health:** Dispatch and Genkit HTTP endpoints when services are running (see root `AGENTS.md`).
