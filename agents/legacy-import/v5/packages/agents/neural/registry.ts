...
import type { CLEAgent, AgentHive, AgentRole, AgentMode, AccessTier } from './agent.js';

class AgentRegistryClass {
    private agents = new Map<string, CLEAgent>();

    /**
     * Register an agent. Called by each hive's index.ts on boot.
     */
    register(agent: CLEAgent): void {
     ...
     */
    get(name: string): CLEAgent | undefined {
     ...
     */
    getHive(hive: AgentHive): CLEAgent[] {
     ...
     * Get agents by role
     */
    getByRole(role: AgentRole): CLEAgent[] {
     ...
     */
    getByMode(mode: AgentMode): CLEAgent[] {
     ...
     */
    getByTier(tier: AccessTier): CLEAgent[] {
