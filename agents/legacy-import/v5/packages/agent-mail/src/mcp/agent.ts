    ...
    this.tools.set('send_email', {
      ...
      handler: async (params) => {
        ...
        if (this.env.RESEND_API_KEY) {
          const oversightBcc = this.env.OVERSIGHT_BCC || null;
          await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${this.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ from: this.env.FROM_EMAIL, to, subject, text: body_text, html: body_html, ...(oversightBcc ? { bcc: oversightBcc } : {}) }),
          });
        }
      ...
      handler: async (params) => {
        ...
        if (this.env.RESEND_API_KEY) {
          ...
          await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${this.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ from: this.env.FROM_EMAIL, to: replyTo, subject: `Re: ${original.subject}`, text: body_text, ...(oversightBcc ? { bcc: oversightBcc } : {}) }),
          });
        }
      ...
      handler: async (params) => {
        ...
        if (this.env.RESEND_API_KEY) {
          ...
          await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${this.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ from: this.env.FROM_EMAIL, to: draft.to_addr, subject: draft.subject, text: draft.body_text, ...(oversightBcc ? { bcc: oversightBcc } : {}) }),
          });
        }
