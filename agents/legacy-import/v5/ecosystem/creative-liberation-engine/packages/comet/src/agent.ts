/**
 * NAVD — Autonomous Task Consumer
 *
 * Background agent loop that wires NAVD into the Creative Liberation Engine dispatch system.
 * Polls the dispatch REST API for tasks with workstream=comet-browser, executes
 * them using the full NAVD pipeline (Router → ExecutionAgent → Validator),
 * and marks them complete with structured results.
 *
 * Entry point: `npm run agent` (runs standalone as a Docker sidecar)
 *
 * Protocol (REST, no shared store — NAVD and dispatch run in separate containers):
 ...
 *   POST  NAVD_URL/execute           { url, instruction, autonomy: 'autonomous' }
...
const NAVD_URL = process.env.NAVD_URL ?? 'http://localhost:7100';
const AGENT_ID = process.env.NAVD_AGENT_ID ?? `comet-agent-${process.pid}`;
const POLL_INTERVAL_MS = parseInt(process.env.NAVD_POLL_MS ?? '10000', 10); // 10s
const PROJECT = process.env.PROJECT ?? 'creative-liberation-engine-v5';
...
async function completeTask(taskId: string, result: TaskResult): Promise<void> {
    try {
        ...
        console.error(`[NAVD/AGENT] Failed to mark task ${taskId} complete:`, err.message);
    }
}
...
async function registerAgent(): Promise<void> {
    try {
        ...
        console.log(`[NAVD/AGENT] ✅ Registered as ${AGENT_ID} at dispatch`);
    } catch {
        console.warn(`[NAVD/AGENT] ⚠️  Dispatch offline — running in standalone mode`);
    }
}
...
function parseBrowseSpec(task: DispatchTask): BrowseSpec | null {
    ...
    console.warn(`[NAVD/AGENT] Could not parse browse spec from task ${task.id}`);
    return null;
}

// ─── Execute via NAVD HTTP API ───────────────────────────────────────────────

async function executeBrowseTask(spec: BrowseSpec): Promise<TaskResult> {
    try {
        console.log(`[NAVD/AGENT] 🌐 Browsing: ${spec.url}`);
        console.log(`[NAVD/AGENT] 📋 Instruction: ${spec.instruction.slice(0, 80)}`);

        const res = await fetch(`${NAVD_URL}/execute`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                url: spec.url,
        ...
        if (!res.ok) {
            ...
            return { success: false, error: `NAVD /execute returned ${res.status}: ${errText.slice(0, 200)}` };
        }
        ...
        if (result.status === 'failed' || result.status === 'blocked') {
            return { success: false, error: result.error ?? `NAVD execution ${result.status}` };
        }
        ...
        if (err.name === 'TimeoutError') {
            return { success: false, error: 'NAVD execution timed out after 3 minutes' };
        }
...
async function agentLoop(): Promise<void> {
    console.log(`
╔══════════════════════════════════════════════════════════════╗
║   NAVD AUTONOMOUS AGENT — Task Consumer                     ║
║   Agent ID: ${AGENT_ID.padEnd(48)}║
╠══════════════════════════════════════════════════════════════╣
║   Dispatch: ${DISPATCH_URL.padEnd(48)}║
║   NAVD:    ${NAVD_URL.padEnd(48)}║
...
╚══════════════════════════════════════════════════════════════╝`);
    ...
    while (running) {
        try {
            ...
            console.log(`\n[NAVD/AGENT] 🎯 Claimed: ${claimed.id} [${claimed.priority}]`);
            console.log(`[NAVD/AGENT]    "${claimed.title.slice(0, 80)}"`);
            ...
            if (result.success) {
                tasksCompleted++;
                console.log(`[NAVD/AGENT] ✅ ${claimed.id} done in ${elapsed}s`);
            } else {
                tasksFailed++;
                console.warn(`[NAVD/AGENT] ❌ ${claimed.id} failed: ${result.error}`);
            }

            console.log(`[NAVD/AGENT] 📊 Done: ${tasksCompleted} | Failed: ${tasksFailed}`);

            // Brief pause between tasks to avoid hammering
            await sleep(2000);

        } catch (err: any) {
            console.error(`[NAVD/AGENT] Loop error:`, err.message);
            currentTaskId = null;
            await sleep(POLL_INTERVAL_MS);
        }
    }
    ...
    console.log('[NAVD/AGENT] 🛑 Shutdown complete');
}
...
process.on('SIGTERM', () => {
    running = false;
    console.log('[NAVD/AGENT] SIGTERM — draining current task then shutting down...');
});
process.on('SIGINT', () => {
    running = false;
    console.log('[NAVD/AGENT] SIGINT — shutting down...');
});

agentLoop().catch(err => {
    console.error('[NAVD/AGENT] Fatal:', err);
    process.exit(1);
});
