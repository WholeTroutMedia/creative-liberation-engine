 ...
 *   adapter.on('event', (e: CLEEvent) => renderer.update(e));
 *   adapter.start();
 */

import { EventEmitter } from 'events';
import type { CLEEvent } from '../types/cle-event.js';

export interface OmnibusAdapterConfig {
  /** Vertical identifier */
  vertical: CLEEvent['vertical'];
...
export abstract class OmnibusAdapter extends EventEmitter {
  ...
  /**
   * Emit an CLEEvent, respecting the rate limiter.
   * Subclasses call this instead of emit() directly.
   */
  protected emitEvent(event: CLEEvent): void {
    const now = Date.now();
