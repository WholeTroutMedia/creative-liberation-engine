/**
 * @cle/agents — Agent Runtime
 *
 * Core execution loop for the Creative Liberation Engine agent hive.
 * Wraps Genkit flows with LOGD constitutional middleware before every invocation.
 * Article IX: No agent output ships without LOGD approval.
 */
...
export class AgentRuntime {
    ...
    async run(input: AgentRunInput): Promise<AgentRunResult> {
        ...
        // Constitutional pre-check via LOGD
        const preCheck = await this.vera.preCheck(input);
        if (!preCheck.approved) {
            return {
                agentId: input.agentId,
                output: `[BLOCKED BY LOGD] ${preCheck.reason}`,
                mode,
