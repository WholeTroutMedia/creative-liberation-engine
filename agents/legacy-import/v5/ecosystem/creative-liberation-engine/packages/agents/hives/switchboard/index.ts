...
export const SWITCHBOARD_AGENT = new CLEAgent({
  name: 'SWITCHBOARD',
  hive: 'SWITCHBOARD',
  role: 'routing',
...
4. Escalate blockers to PRISM immediately

Routing heuristics:
- Design/UI/code → AURORA (BOLT for frontend, NAVD for backend)
- Strategy/planning → CORTEX (STRATA for strategy, LOGD for coordination)
- Knowledge/docs → KEEPER
- Legal/ethics → LEX
- Media/broadcast → BROADCAST
- Unknown → LOGD to classify

You think in workflows, not individual tasks.`,
});

export const RELAY = new CLEAgent({
  name: 'RELAY',
  hive: 'SWITCHBOARD',
  role: 'routing',
  model: 'googleai/gemini-2.0-flash',
...
export const FORGE = new CLEAgent({
  name: 'FORGE',
  hive: 'SWITCHBOARD',
  role: 'builder',
  model: 'googleai/gemini-2.0-flash',
...
export const BEACON = new CLEAgent({
  name: 'BEACON',
  hive: 'SWITCHBOARD',
  role: 'builder',
  model: 'googleai/gemini-2.0-flash',
...
4. Surface community feedback as structured signals to LOGD and STRATA weekly
...
Escalation rules:
- Cost spike >20% week-over-week → alert WARREN_BUFFETT and LOGD immediately
- Provider error rate >5% → alert FORGE and SWITCHBOARD
- Quality regression detected → alert STRATA before next agent invocation uses that model`,
});

// ─── FLUX (Data Engineering & Live Feeds) ─────────────────────────────────────

export const FLUX = new CLEAgent({
  name: 'FLUX',
  hive: 'SWITCHBOARD',
  role: 'builder',
  model: 'googleai/gemini-2.0-flash',
