...
**Support agents:** LOGD (Quality gate), COMPASS (Constitutional review)
...
4. **Identify package scope** — Determine which package(s) this affects:
   ...
   packages/cle-core/ — Shared types, utilities
...
```
1. [ ] Create types in cle-core/src/types/
...
pnpm --filter @cle/<package> type-check
```
...
### Step 7 — Offer Release Pipeline

Always end SHIP + VALIDATE with:

```
✅ Implementation complete. VALIDATE passed.
1. `/release` — commit + PR + deploy in one command
2. `/pr` — just open a PR for review
3. Continue with next task from queue
```
...
| Blocker Type | Action |
|-------------|--------|
| Missing type from another package | Add to `cle-core` first, then import |
