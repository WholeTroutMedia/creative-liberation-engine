---
name: SCRIBE Memory
description: SCRIBE is the persistent memory layer of the Creative Liberation Engine. Use this skill when you need to write a significant decision, pattern, or discovery to long-term memory, or when KEEPER needs to retrieve relevant prior context for planning or ideation.
---
...
**Read from memory (KEEPER IN) when:**
...
## Reading from Memory (KEEPER Protocol)
...
Get-ChildItem "D:\Google Antigravity\Infusion Engine Brainchild\creative-liberation-engine-v4\creative_liberation_engine\memory\scribe_archive\" -Recurse -Filter "*.md" | Select-String "<query>"
```
...
```powershell
# Semantic memories
$path = "D:\Google Antigravity\Infusion Engine Brainchild\creative-liberation-engine-v4\creative_liberation_engine\memory\scribe_archive\semantic\"

# Episodic memories
$path = "D:\Google Antigravity\Infusion Engine Brainchild\creative-liberation-engine-v4\creative_liberation_engine\memory\scribe_archive\episodic\"
```
...
When KEEPER retrieves memories that are clearly outdated (e.g., reference old package names, deprecated patterns, or v4 architecture that's been replaced in v5):
...
| Mode | SCRIBE Action |
|------|--------------|
| IDEATE | KEEPER retrieves at start; SCRIBE writes direction selection at end |
| PLAN | KEEPER retrieves architecture context at start; SCRIBE writes approved plan |
