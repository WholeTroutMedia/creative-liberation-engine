...
3. POST to GHOST browser agent for visual regression check (if UI changed)
...
- `packages/genkit/**` â†’ `@cle/genkit`
- `packages/genmedia/**` â†’ `@cle/genmedia`
- `apps/console/**` â†’ `console` (React app)
- `packages/finance-agent/**` â†’ `@cle/finance-agent`
- etc.
...
- POST to `http://localhost:6000/scan` (GHOST QA agent):
  ```json
  { "url": "http://localhost:5173", "changedComponent": "<component name>" }
  ```
- If GHOST is not running: skip and note "[QA] GHOST unavailable â€” manual visual check recommended"
...
[SHADOW QA] âœ“ Visual: GHOST scan clean
