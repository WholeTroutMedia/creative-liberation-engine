---
description: Activate VALIDATE mode â€” LOGD + COMPASS run full quality assurance, constitutional compliance, and launch readiness checks. The 4th mode in the IDEATEâ†’PLANâ†’SHIPâ†’VALIDATE cycle. Final gate before /release.
---

# /validate-mode [feature]

The full **VALIDATE** operational mode â€” distinct from `/validate` (pre-commit diff review). This is the launch readiness gate after SHIP completes. LOGD + COMPASS run all 6 quality sections and produce a structured verdict report.
...
### Section 3 â€” Constitutional Review (COMPASS)
...
1. **On LAUNCH READY** â€” offer:

   ```
   âœ… VALIDATE complete â€” LAUNCH READY

   1. `/release` â€” commit + PR + deploy (full Article XX pipeline)
   2. `/pr` â€” open a PR for human review first
   3. Next task â€” pick up the next item from the dispatch queue
   ```
...
- Lead: LOGD (Analyst) + COMPASS (Constitutional Guardian)
