...
2. Ask the user (one question only):
   > "What should the next instance know about the current state of `[workstream]`? (Files changed, next steps, anything blocked?)"
   >
   > If the user's message already contains a handoff note, skip this question and use their note directly.
