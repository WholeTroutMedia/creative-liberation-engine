---
description: Activate IDEATE mode â€” KEEPER + STRATA explore a topic, map the possibility space, generate creative directions, and prime HANDOFF.md for PLAN mode
---

# /ideate <topic>

Enters **IDEATE** operational mode. KEEPER retrieves institutional memory, STRATA + PRISM generate 5 creative directions, and on direction selection, HANDOFF.md is written to prime PLAN mode automatically.
...
4. **Offer exactly 3 next actions:**

   ```
   1. "Generate UI screens for [direction]" â†’ /design <direction-summary>
   2. "Build the implementation plan" â†’ /plan <topic>
   3. "Explore a different angle" â†’ ask which of the 5 to pivot from, re-run Step 5
   ```
...
- KEEPER runs before generation so STRATA's strategy is informed by institutional memory
- Do NOT auto-proceed to PLAN â€” present options and wait for user direction
