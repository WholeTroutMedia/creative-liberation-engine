...
class BOLTAgent(BaseAgent):
    ...
    def execute(self, context: Dict[str, Any]) -> AgentResult:
        ...
        if task_type == "frontend":
            return AgentResult(success=True, output=self._build_frontend(task, context))
        elif task_type == "ios":
            return AgentResult(success=True, output=self._build_ios(task, context))
        elif task_type == "component":
            return AgentResult(success=True, output=self._build_component(task, context))
        elif task_type == "integration":
            return AgentResult(success=True, output=self._integrate_api(task, context))
        else:
            return AgentResult(success=True, output=self._general_development(task, context))
