...
class SYSTEMSAgent(BaseAgent):
    ...
    def execute(self, context: Dict[str, Any]) -> AgentResult:
        ...
        if task_type == "deploy":
            return AgentResult(success=True, output=self._deploy_application(task, context))
        elif task_type == "infrastructure":
            return AgentResult(success=True, output=self._setup_infrastructure(task, context))
        elif task_type == "cicd":
            return AgentResult(success=True, output=self._setup_cicd(task, context))
        elif task_type == "monitoring":
            return AgentResult(success=True, output=self._setup_monitoring(task, context))
        else:
            return AgentResult(success=True, output=self._general_devops(task, context))
