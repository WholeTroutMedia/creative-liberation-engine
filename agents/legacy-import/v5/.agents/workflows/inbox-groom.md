...
4. **Duplicate Checks & Balances Validation**
   Before creating a new IDEATE artifact, run a `grep_search` across `apps/ideate-sessions/` to see if the core concepts, keywords, or general topic of the new article have already been groomed into an existing IDEATE session.
   - If a highly similar session already exists, DO NOT create a new duplicate file.
   - Instead, append your novel findings or trajectories to the existing file.
