...
Use this when you have a large goal that should be executed by multiple Antigravity instances in parallel. AVERI decomposes the goal into atomic, parallel-safe workstreams and writes them to the task queue.
...
2. **Decompose the goal** into parallel-safe tasks. Rules:
   - Each task must be completable by one Antigravity instance independently
