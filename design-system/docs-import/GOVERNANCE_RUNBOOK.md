...
```bash
pnpm --filter @cle/design-governance scan
pnpm --filter @cle/design-agent scan:typography
pnpm --filter @cle/design-agent scan:layout
```
...
2. Run `pnpm --filter @cle/design-governance report` — shows all usages.
...
pnpm --filter @cle/design-tokens penpot:pull

# Push compiled tokens to Penpot library
pnpm --filter @cle/design-tokens penpot:push
```
